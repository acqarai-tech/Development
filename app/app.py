...
# # ✅ New: Override table for market pricing
...
# # ✅ ADDED: range band for low/high (default ±15%)
...
# # Anchor calibration globals
# ANCHOR_GROUP: Optional[str] = None
# AREA_COL_FOR_TOTAL: str = "procedure_area"
# ANCHOR_LOOKUP: Dict[str, Dict[str, Any]] = {}

# # ✅ Multi-level anchors from TX (fresh)
...
# # ✅ Market overrides from Supabase table
# PROJECT_OVERRIDES: Dict[str, Dict[str, Any]] = {}

# # ✅ Calibration defaults (safe)
# CAL: Dict[str, Any] = {
#     "prudence": 0.80,          # anchor = prudence*p50 + (1-prudence)*p80
#     "blend_anchor": 0.65,
#     "blend_model": 0.35,
#     "clamp_low": 0.85,
#     "clamp_high": 1.15,
...
#     """Convert procedure_area to sqm if user sends sqft."""
#     area = float(user_data.get("procedure_area", 0) or 0)
...
# def predict_price_per_sqm_raw(user_data: Dict[str, Any]) -> float:
#     """Model output = price_per_sqm (AED/sqm)."""
#     if bundle is None:
#         raise RuntimeError("Model bundle not loaded")
...
# def _build_anchor_lookup(anchor_stats: Optional[pd.DataFrame], anchor_group: Optional[str]) -> Dict[str, Dict[str, Any]]:
#     """
#     Bundle anchor lookup (project-only, normalized)
#     """
#     if anchor_stats is None or anchor_group is None:
#         return {}
#     if not isinstance(anchor_stats, pd.DataFrame) or anchor_stats.empty:
...
#     # if anchor is weak, rely more on model
...
#     # linear between p50 (0.50) and p80 (0.80), then allow a bit more up to 0.95
#     if cap_q <= 0.80:
...
# # ✅ ADDED: compute low/high range from total valuation
...
#     """
#     Market value logic:
#     - If override exists: use it (market_psm or market_multiplier)
#     - Else if anchor is strong (project/master): market = tx (medium confidence)
#     - Else (area/city): market = tx proxy (low confidence)
#     """
#     proj = _norm_key(user_data.get("project_name_en"))
...
#     if "price_per_sqm" not in df.columns or df["price_per_sqm"].isna().all():
#         # fallback if you only have meter_sale_price
#         if "meter_sale_price" in df.columns:
#             df["price_per_sqm"] = pd.to_numeric(df["meter_sale_price"], errors="coerce")
...
#         # adopt calibration from bundle (overrides defaults)
#         if isinstance(bundle.get("ovaluate_calibration"), dict):
...
#     # 2) Load transactions + build anchors
#     try:
#         tx = _load_tx_from_supabase()
#         if tx.empty:
...
#     # ✅ ADDED: send low/high range (based on market total)
#     rng = _range_from_total(market_total, RANGE_BAND)

#     payload = {
#         "currency": CURRENCY,

#         # ✅ Transaction-based (resale AVM)
#         "tx": {
#             "price_per_sqm": tx_psm,
#             "price_per_sqft": tx_psf,
#             "total_valuation": tx_total,
#             "anchor_level": level,
#         },

#         # ✅ Market-based (competitor-like) using overrides when needed
#         "market": {
#             "price_per_sqm": market_psm,
#             "price_per_sqft": market_psf,
#             "total_valuation": market_total,
#             "method": m.get("method"),
#             "confidence": m.get("confidence"),
#             "override_used": bool(m.get("override")),
#             "override": m.get("override") or None,
#         },
...
def get_comparables(user_data: Dict[str, Any], top_k: int = 10):
    ...
    if area and "area_name_en" in df.columns:
        df_area = df[df["area_name_en"].astype(str).str.contains(area, case=False, na=False)].copy()
        if len(df_area) >= 3:
            df = df_area
