...
# # Safe: if .env doesn't exist (Render), this won't crash
...
# # -------------------------------------------------
# # Model download (Supabase Storage) config
# # -------------------------------------------------
# # ✅ Keep bucket PRIVATE. Use SERVICE ROLE KEY in backend env only.
...
# # -------------------------------------------------
# # Globals loaded on startup (TX only)
# # Model is LAZY-LOADED on first predict request
# # -------------------------------------------------
# bundle: Optional[dict] = None
# feature_cols: List[str] = []
# LOG_TARGET: bool = False
...
# # -------------------------------------------------
# # ✅ Model: download + lazy load (new functionality)
# # -------------------------------------------------
# def _ensure_model_file_present():
#     """
#     Ensure MODEL_PATH exists locally.
#     If missing, download from Supabase Storage (private bucket) using SERVICE ROLE KEY.
#     """
...
#         raise RuntimeError(
#             "Model file missing locally and download is not configured. "
#             "Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in Render env."
#         )

#     sb = create_client(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY)
#     data = sb.storage.from_(MODEL_BUCKET).download(MODEL_OBJECT)
...
#     """
#     Load the joblib bundle only once (lazy).
#     Keeps your existing globals and behavior the same.
#     """
...
#     _ensure_model_file_present()

#     b = joblib.load(str(MODEL_PATH))
#     bundle = b
#     feature_cols = list(b.get("feature_columns", []))
#     LOG_TARGET = bool(b.get("log_target", False))
#     DATE_COL = b.get("date_parts_from", "instance_date")
#     num_cols = list(b.get("numeric_columns", []))
#     cat_cols = list(b.get("categorical_columns", []))
...
#     # keep same cleanup as your previous loader
#     if "trans_group_en" in df.columns:
#         # FIX: Supabase returns "Sales" (capital S) in your query
#         df = df[df["trans_group_en"].astype(str).str.lower() == "sales"].copy()
...
# def _startup():
#     """
#     ✅ Startup loads ONLY tx.
#     ✅ Model is NOT loaded on startup (prevents 512MB Render crash).
#     """
#     global bundle, feature_cols, LOG_TARGET, DATE_COL, num_cols, cat_cols, tx

#     STARTUP_WARNINGS.clear()

#     # --- model: do NOT load here
#     bundle = None
#     feature_cols = []
#     LOG_TARGET = False
...
#             "Will download from Supabase Storage on first prediction (lazy load)."
#         )
...
# def health():
#     return {
#         "status": "ok" if (bundle is not None or not tx.empty) else "degraded",
#         "model_loaded": bundle is not None,
...
# 🔹 LOAD .env (local dev only). Railway/Render uses dashboard env vars.
from dotenv import load_dotenv
ENV_PATH = Path(__file__).resolve().parent / ".env"
...
import numpy as np
import pandas as pd
import requests  # ✅ REQUIRED (you use requests.get in _load_tx)
...
# -------------------------------------------------
# Supabase config (reads from Railway/Render env OR local .env)
# -------------------------------------------------
SUPABASE_URL = os.getenv("SUPABASE_URL", "").rstrip("/")
SUPABASE_ANON_KEY = os.getenv("SUPABASE_ANON_KEY", "")
TX_TABLE = os.getenv("TX_TABLE", "transactions_avm")
...
# -------------------------------------------------
# Globals (TX loads on startup, model loads lazily on first predict)
# -------------------------------------------------
bundle: Optional[dict] = None
feature_cols: List[str] = []
LOG_TARGET: bool = False
...
# -------------------------------------------------
# ✅ Lazy model loader (Railway-friendly)
# -------------------------------------------------
def _load_model_once():
    """
    Loads the joblib bundle only when needed (first /predict call).
    Prevents Railway healthcheck failures due to heavy startup.
    """
    global bundle, feature_cols, LOG_TARGET, DATE_COL, num_cols, cat_cols
...
def _load_tx() -> pd.DataFrame:
    """
    Loads transactions.
    - Prefers Supabase REST if SUPABASE_URL + SUPABASE_ANON_KEY exist
    - Falls back to local CSV if missing or Supabase fails
    """
    if not SUPABASE_URL or not SUPABASE_ANON_KEY:
        STARTUP_WARNINGS.append(
            "Supabase env missing (SUPABASE_URL / SUPABASE_ANON_KEY). Falling back to local TX file."
        )
        return _load_tx_from_local_file()
...
def _startup():
    """
    ✅ Start fast for Railway healthcheck:
    - Do NOT load model here
    - Load TX, but never crash startup if it fails
    """
    global bundle, feature_cols, LOG_TARGET, DATE_COL, num_cols, cat_cols, tx

    STARTUP_WARNINGS.clear()

    # --- model: don't load on startup
    bundle = None
    feature_cols = []
    LOG_TARGET = False
    DATE_COL = "instance_date"
    ...
    try:
        ...
        STARTUP_WARNINGS.append(f"TX load failed at startup: {e}")
        tx = pd.DataFrame()
...
def debug_area_values(inp: PropertyInput):
    ...
    if not area:
        return {"count": 0, "values_sample": []}
    vals = tx.loc[
        tx["area_name_en"].astype(str).str.contains(area, case=False, na=False),
        "area_name_en"
...
def health():
    return {
        "status": "ok" if (not tx.empty or bundle is not None) else "degraded",
        "model_loaded": bundle is not None,
        "model_path": str(MODEL_PATH),
        "tx_loaded": not tx.empty,
        "tx_source": "supabase" if (SUPABASE_URL and SUPABASE_ANON_KEY) else "local_file",
