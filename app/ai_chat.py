...
#     # If this is a short follow-up (e.g. just an area name) and history shows
#     # a prior intent (sell/buy/invest/broker), merge it in so context isn't lost.
#     if len(message.split()) <= 6 and req.history:
#         last_user_msgs = [h.get("content", "") for h in req.history if h.get("role") == "user"]
#         if last_user_msgs:
#             prior = last_user_msgs[-1].lower()
#             prior_area_id, _ = get_area_id(prior)
#             current_area_id, _ = get_area_id(msg_lower)
#             # Only merge if the prior message had no area (was the vague trigger)
#             # and this message supplies one, and prior had a clear role intent.
