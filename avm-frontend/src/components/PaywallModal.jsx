...
function CheckoutForm({ onSuccess, onError, userDetails }) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // First try to sign in (maybe account already exists)
      ...
      if (signInData?.session) {
        ...
        console.log("[PaywallModal] Existing user logged in:", userId);
      } else {
        // ✅ New user — create account
        console.log("[PaywallModal] Creating new account for:", userDetails.email);
        ...
        const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
          ...
          options: {
            ...
            // IMPORTANT: disable email confirmation so session is returned immediately
            // This requires "Enable email confirmations" to be OFF in Supabase Auth settings
            // OR handle the confirmation flow separately
          },
        });

        if (signUpError) {
          console.error("[PaywallModal] Signup error:", signUpError);
          setErrMsg(signUpError.message);
          setLoading(false);
          return;
        }

        console.log("[PaywallModal] Signup response:", signUpData);
        ...
        if (!userSession && signUpData.user) {
          // User is created in auth but not confirmed yet
          // We still have the user.id — insert into users table directly
          userId = signUpData.user.id;
          console.log("[PaywallModal] Email confirmation required, userId:", userId);
        }
      }
...
{/* Role */}
            ...
            <CheckoutForm
              onSuccess={onSuccess}
              userDetails={{ name, email, phone, countryCode, password,role }}
            />
