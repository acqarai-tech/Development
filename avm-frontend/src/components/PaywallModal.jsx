...
// // ── Inner checkout form ──────────────────────────────────────────────────────
// function CheckoutForm({ onSuccess, onError, userDetails, isLoggedIn, paymentIntentId, setClientSecret, setPaymentIntentId, valuationId, termsAccepted, setTermsAccepted, existingUserId, isFree }) {
...
function CheckoutForm({ onSuccess, onError, userDetails, isLoggedIn, paymentIntentId, setClientSecret, setPaymentIntentId, valuationId, termsAccepted, setTermsAccepted, existingUserId, isFree }) {
  ...
  const handlePay = async () => {
    ...
    // ── Validate fields ──
    if (!isLoggedIn && !existingUserId) {
      if (!userDetails.name.trim()) return setErrMsg("Please enter your full name.");
      if (!userDetails.email.trim()) return setErrMsg("Please enter your email address.");
      if (!userDetails.phone.trim()) return setErrMsg("Please enter your phone number.");
      if (!userDetails.role) return setErrMsg("Please select your role.");
    }
    ...
    // ─────────────────────────────────────────────────────────────────
    // DISCOUNT PATH — runs completely independently, never touches Stripe
    // ─────────────────────────────────────────────────────────────────
    if (discountApplied) {
      try {
        let userId = null;
        ...
        // Insert user row for new signups
        if (!isLoggedIn && !existingUserId) {
        ...
        // Upgrade to pro — no Stripe involved
        const { error: upgradeError } = await supabase.from("users").update({
          plan: "pro",
          account_type: "pro",
          free_reports_limit: 10,
      ...
      return; // ← stops here, never reaches Stripe below
    }

    // ─────────────────────────────────────────────────────────────────
    // NORMAL STRIPE PAYMENT PATH
    // ─────────────────────────────────────────────────────────────────
    try {
      let userId = null;
      ...
      if (!isLoggedIn && !existingUserId) {
        ...
        if (insertError) console.error("[PaywallModal] Users table insert error:", insertError);
        else console.log("[PaywallModal] User row created/updated in users table ✅");
      }
      ...
      // Update receipt email before payment
      if (userDetails.email) {
        try {
          await fetch(
            `${process.env.REACT_APP_SUPABASE_URL}/functions/v1/update-payment-email`,
        ...
        confirmParams: {
          ...
          payment_method_data: {
            billing_details: {
              email: userDetails.email?.trim() || "",
              name: userDetails.name?.trim() || "",
              phone: userDetails.phone ? `${userDetails.countryCode}${userDetails.phone.trim()}` : "",
              address: { country: "AE", postal_code: "", line1: "", city: "", state: "" },
            },
          },
        },
        redirect: "if_required",
      ...
      if (paymentIntent?.status === "succeeded") {
        ...
        await fetch(
          `${process.env.REACT_APP_SUPABASE_URL}/functions/v1/save-payment-details`,
          {
            ...
            body: JSON.stringify({ paymentIntentId: paymentIntent.id, userId: userId }),
          }
        );
        ...
        if (upgradeError) console.error("[PaywallModal] Plan upgrade error:", upgradeError);
        else console.log("[PaywallModal] Plan upgraded to pro ✅");

        if (existingUserId && !isLoggedIn) {
          await supabase.auth.signInWithOtp({ email: userDetails.email.trim() });
          setShowSuccess(true);
          setTimeout(() => { setShowSuccess(false); window.location.href = "/dashboard"; }, 3000);
          return;
        }
  ...
  return (
    <div>
      ...
      {showSuccess && (
        <div style={{
          ...
          <div style={{
            ...
            boxShadow: "0 24px 60px rgba(0,0,0,0.3)", animation: "fadeInUp 0.4s ease",
          }}>
            <div style={{
              width: 72, height: 72, borderRadius: "50%",
              background: "rgba(34,197,94,0.1)", border: "3px solid rgba(34,197,94,0.4)",
              display: "flex", alignItems: "center", justifyContent: "center",
              margin: "0 auto 20px",
            }}>
              <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <h2 style={{ fontSize: 22, fontWeight: 900, color: "#1a1a1a", marginBottom: 8, letterSpacing: "-0.02em" }}>
              Payment Successful! 🎉
            </h2>
            ...
            <div style={{
              ...
              padding: "8px 20px", background: "#FFF7ED", border: "1px solid #F5C89A",
              borderRadius: 999, marginBottom: 20,
            }}>
              <span style={{ fontSize: 16 }}>⭐</span>
              <span style={{ fontSize: 12, fontWeight: 800, color: "#B87333", textTransform: "uppercase", letterSpacing: "0.1em" }}>
                Founding Member — Pro Plan
              </span>
            </div>
            <div style={{ height: 4, background: "#f3f4f6", borderRadius: 999, overflow: "hidden" }}>
              <div style={{
                height: "100%", background: "linear-gradient(to right, #B87333, #D4956A)",
                borderRadius: 999, animation: "progressBar 3s linear forwards",
              }} />
            </div>
            <style>{`
      ...
      {/* ── Card form — hidden when discount applied ── */}
      {!isFree && !discountApplied && (
        <PaymentElement
          options={{
            fields: {
      ...
      <button
        onClick={handlePay}
        disabled={(!isFree && !discountApplied && !stripe) || loading}
        style={{
          display: "block", margin: "24px auto 0",
          width: "min(100%, 400px)", padding: "16px 28px",
          background: loading ? "#ccc" : (isFree || discountApplied) ? "#16a34a" : "#B87333",
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          background: "linear-gradient(135deg, #FFF7ED 0%, #FEF3E2 100%)",
          border: "1px solid #F5C89A", borderRadius: 14, padding: "20px 24px",
          display: "flex", justifyContent: "space-between", alignItems: "center",
          marginBottom: 32, boxShadow: "0 2px 16px rgba(184,115,51,0.15)",
          ...
          <div>
            ...
            {continueError === "PRO_ACCOUNT" && (
              <div style={{
                ...
                <div style={{
                  ...
                  <button
                    onClick={() => { setContinueError(""); window.location.href = "/login"; }}
                    style={{ width: "100%", padding: "14px", background: "#B87333", color: "#fff", borderRadius: 10, border: "none", fontWeight: 800, fontSize: 14, cursor: "pointer", marginBottom: 10 }}
                  >
                    Log In to My Account
                  </button>
                  <button
                    onClick={() => setContinueError("")}
                    style={{ width: "100%", padding: "12px", background: "none", border: "1px solid #e5e7eb", borderRadius: 10, fontWeight: 600, fontSize: 13, color: "#888", cursor: "pointer" }}
                  >
                    Use a Different Email
                  </button>
                </div>
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  const { data: existingUser } = await supabase
                    .from("users").select("id, plan, email")
                    .eq("email", email.trim().toLowerCase())
                    .maybeSingle();

                  if (existingUser) {
                    if (existingUser.plan === "pro") {
                      setContinueError("PRO_ACCOUNT");
                      setContinueLoading(false);
                      return;
                    }
                    setExistingUserId(existingUser.id);
