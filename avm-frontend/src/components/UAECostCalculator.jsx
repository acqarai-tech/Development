...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif",background:"#F2F1EF",minHeight:"100vh",padding:"24px 12px" }}>
      <div style={{ maxWidth:680,margin:"0 auto" }}>
        ...
        <div style={{ background:"#fff",borderRadius:18,border:"1px solid #E8E8E8",overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,.07)" }}>
          ...
          <div style={{ padding:"22px 24px 0" }}>
            <div style={{ display:"flex",gap:3,background:"#F5F5F5",borderRadius:9,padding:3 }}>
              {[["buy","🏦  Buying Costs"],["sell","🤝  Selling Costs"]].map(([id,lbl])=>(
                <button key={id} onClick={()=>setTab(id)} style={{ flex:1,padding:"10px 0",border:"none",borderRadius:7,fontSize:12,fontWeight:800,letterSpacing:".06em",textTransform:"uppercase",cursor:"pointer",transition:"all .15s",background:tab===id?"#2B2B2B":"transparent",color:tab===id?"#fff":"rgba(43,43,43,.4)" }}>{lbl}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ padding:"22px 24px 28px" }}>
            ...
            <div style={{ marginBottom:18 }}>
              ...
              <div style={{ display:"flex",gap:5,marginTop:8,flexWrap:"wrap" }}>
                {[500000,1000000,2000000,5000000,10000000].map(v=>(
                  <button key={v} onClick={()=>setRaw(v.toLocaleString("en-AE"))} style={{ padding:"4px 10px",borderRadius:5,border:"1px solid #E8E8E8",background:price===v?"#2B2B2B":"#FAFAFA",color:price===v?"#fff":"rgba(43,43,43,.55)",fontSize:11,fontWeight:700,cursor:"pointer" }}>
            ...
            {/* Options */}
            <div style={{ background:"#FAFAFA",border:"1px solid #F0F0F0",borderRadius:11,padding:"14px 16px",marginBottom:20 }}>
                    ...
                    {mort&&(
                      <div style={{ gridColumn:"1/-1",paddingTop:4 }}>
                        ...
                        <div style={{ display:"flex",justifyContent:"space-between",fontSize:10,color:"rgba(43,43,43,.32)",marginTop:2 }}>
                          <span>25%</span><span>UAE CB max: 75% expat / 80% national (ready property)</span><span>85%</span>
                        </div>
                      </div>
                    )}
                  </>
                    ...
                    <div style={{ gridColumn:"1/-1" }}>
                      ...
                      <div style={{ display:"flex",gap:7 }}>
                        {[{id:"low",lbl:"Low",sub:"AED 500",desc:"Small devs"},{id:"typical",lbl:"Typical",sub:"AED 2,000",desc:"Major devs"},{id:"high",lbl:"High",sub:"AED 5,000",desc:"Premium devs"}].map(o=>(
                          <button key={o.id} onClick={()=>setNoc(o.id)} style={{ flex:1,padding:"8px 4px",borderRadius:8,cursor:"pointer",border:`2px solid ${noc===o.id?"#B87333":"#E8E8E8"}`,background:noc===o.id?"#FFF8F3":"#fff",textAlign:"center" }}>
            ...
            {!Number.isFinite(price)?(
              ...
              <>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:18 }}>
                  {[
                    ...
                    <div key={c.lbl} style={{ background:c.dark?"#2B2B2B":"#FAFAFA",border:`1px solid ${c.dark?"#2B2B2B":"#EBEBEB"}`,borderRadius:9,padding:"13px 14px" }}>
                      ...
                      <div style={{ fontSize:"clamp(12px,2.2vw,17px)",fontWeight:800,color:c.dark?"#fff":"#1a1a1a",fontFamily:"monospace",lineHeight:1.1 }}>{c.val}</div>
                    ...
                    return(
                      <div key={r.label} style={{ marginBottom:6 }}>
                        <div style={{ display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:3 }}>
                          <span style={{ color:"rgba(43,43,43,.6)",fontWeight:500 }}>{r.label}</span>
                          <span style={{ fontWeight:700,fontFamily:"monospace",color:"#2B2B2B" }}>{p.toFixed(1)}% · {fmt(r.amount)}</span>
                        </div>
                ...
                {/* Round trip */}
                {buying&&selling&&(
                  <div style={{ background:"#1a1a1a",borderRadius:11,padding:"13px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
                    <div>
