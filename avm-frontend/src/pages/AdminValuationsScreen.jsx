...
//     // Fetch feedback valuation_ids to check which reports have feedback
//     const {data: fbData} = await supabase
//       .from('feedback')
//       .select('valuation_id')
//       .not('valuation_id', 'is', null);

//     const fbSet = new Set((fbData || []).map(f => String(f.valuation_id)));

//     const enriched = (data || []).map(v => ({
//       ...v,
//       has_feedback: fbSet.has(String(v.id)),
//     }));
...
//                       return (
//                        <tr key={val.id}
//   onClick={() => navigate(`/report?id=${val.id}`)}
//   style={{borderBottom:idx<filtered.length-1?`1px solid ${C.border}`:'none',transition:'background 0.1s', cursor:'pointer'}}
...
//                           {/* Type */}
// <td style={{padding:'20px 16px'}}>
//   <span style={{padding:'3px 9px',borderRadius:6,fontSize:9,fontWeight:800,textTransform:'uppercase',letterSpacing:'0.08em',background:'#F3F3F4',color:C.muted,border:`1px solid ${C.border}`,display:'inline-block',whiteSpace:'nowrap'}}>
//     {val.type || 'VALUCHECK'}
//   </span>
// </td>
//                           {/* <td style={{padding:'20px 16px'}}>
...
//     background: val.has_feedback ? '#F0FDF4' : '#F3F3F4',
//     color: val.has_feedback ? '#059669' : C.muted,
//     border: `1px solid ${val.has_feedback ? '#A7F3D0' : C.border}`,
//   }}>
//     {val.has_feedback ? 'Received' : 'None'}
//   </span>
// </td>
