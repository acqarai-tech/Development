...
//   useEffect(() => {
//     // ✅ Synchronous localStorage check — before any async
...
//         // ✅ Sign in admin anonymously so RLS allows reads
//         // Check if there's already a session first
//         const { data: sessData } = await supabase.auth.getSession();

//         if (!sessData?.session) {
//           // Sign in with admin credentials to get a valid session for RLS
//           const { error: signInErr } = await supabase.auth.signInWithPassword({
//             email: "admin@acqar.com",
//             password: "acqar123",
//           });
//           if (signInErr) {
//             // If admin user doesn't exist in Supabase Auth, use anon and rely on RLS policy
//             console.warn("Admin sign-in failed, proceeding with anon:", signInErr.message);
//           }
//         }
...
//   .neq("email", "admin@acqar.com");  // ← exclude by email, not role
...
//         <div style={styles.brand} className="ad-brand" onClick={() => navigate("/admin-dashboard")}>
...
//                 <div style={styles.tableBadge}>{latestUsers.length} records</div>
//               </div>
...
//                 </div>
//                 <div style={styles.tableWrap} className="ad-table">
//                   <div style={styles.thFeedback} className="ad-thVals">
//                     <div>ID</div>
//                     <div>User</div>
//                     <div>Message</div>
//                     <div>Created</div>
...
//                       <div style={styles.bold}>{userNameMap[f.user_id] || "—"}</div>
//                       <div style={styles.muted}>{f.message || "—"}</div>
//                       <div style={styles.muted}>
...
//     .ad-thUsers, .ad-trUsers, .ad-thVals, .ad-trVals { min-width: 780px; }
//     .ad-brand { width: 100%; }
//   }
// `;
...
//   },
//   thFeedback: {
//     display: "grid", gridTemplateColumns: "0.5fr 1fr 3fr 1fr",
//     fontWeight: 800, fontSize: 11, letterSpacing: 0.8,
//     padding: "10px 14px", background: "#f9fafb",
//     borderRadius: "10px 10px 0 0", color: MUTED,
//     border: `1px solid ${BORDER}`, borderBottom: "none", textTransform: "uppercase",
