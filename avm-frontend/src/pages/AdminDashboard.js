...
// src/pages/AdminDashboard.jsx
// ✅ NO functionality change
// ✅ ONLY responsiveness added (no logic changes)
// - cards stack on mobile
// - tables become horizontal-scrollable with a safe min-width
// - topbar wraps nicely
          ...
          <div style={styles.grid} className="ad-grid">
            ...
            <div style={styles.cardWide} className="ad-cardWide">
              <div style={styles.table} className="ad-table">
                ...
                {latestUsers.map((u) => (
                  <div key={u.id} style={styles.trUsers} className="ad-trUsers">
                    <div style={styles.mono}>{String(u.id).slice(0, 8)}</div>
                    <div style={styles.bold}>{u.name || "—"}</div>
                    <div style={styles.mono}>{u.email || "—"}</div>
                    <div>
            ...
            <div style={styles.cardWide} className="ad-cardWide">
              <div style={styles.table} className="ad-table">
                ...
                {latestValuations.map((v) => {
                  ...
                  return (
                    <div key={v.id} style={styles.trVals} className="ad-trVals">
                      <div style={styles.mono}>#{v.id}</div>
                      <div style={styles.bold}>{userName}</div>
                      <div style={styles.bold}>{title}</div>
                      <div>{v.district || "—"}</div>
...
/* ✅ ONLY responsiveness added below (colors untouched) */
const responsiveCss = `
  ...
  @media (max-width: 820px) {
    ...
    .ad-card{
      grid-column: span 12 !important; /* stack 3 cards */
    }
    .ad-cardWide{
  ...
  /* Tables: keep current grid layout but ensure it scrolls on small screens */
  @media (max-width: 640px) {
    ...
    /* Force a minimum width so columns don't crush */
