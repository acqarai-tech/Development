...
//     points.push({
//       year: String(currentYear - i),
//       historical: Math.round(currentValue / Math.pow(1 + GROWTH_RATE, i)),
//       forecast: null,
//     });
//   }
...
//     historical: Math.round(currentValue),
...
//       year: String(currentYear + i),
//       historical: null,
//       forecast: Math.round(currentValue * Math.pow(1 + GROWTH_RATE, i)),
//     });
//   }
...
//                 return [fmtAED(v), name === "historical" ? "Historical" : "AI Forecast"];
//               }}
//               contentStyle={{ borderRadius: 10, border: "1px solid #e5e7eb", fontSize: 12 }}
//             />
...
//       await supabase.from("no_data_logs").insert({
//         user_id: userId,
...
//   return dateB - dateA;  // ← reversed: recent first
// });
//   }, [reportData, formData]);
...
//     {/* ── AMENITIES from valRow.features (Supabase) or formData ── */}
//    {(() => {
//   const amenities =
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 18 }}>
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            key={star}
            type="button"
            onClick={() => setFbStar(star)}
            onMouseEnter={() => setFbStarHover(star)}
            onMouseLeave={() => setFbStarHover(0)}
            style={{ background: "none", border: "none", cursor: "pointer", padding: "2px 3px", lineHeight: 1 }}
            aria-label={`Rate ${star} star${star > 1 ? "s" : ""}`}
          >
            <svg
              width="28" height="28" viewBox="0 0 24 24"
              fill={(fbStarHover || fbStar) >= star ? "#B87333" : "none"}
              stroke={(fbStarHover || fbStar) >= star ? "#B87333" : "#D1D5DB"}
              strokeWidth="1.8"
        ...
        {fbStar > 0 && (
          <span style={{ fontSize: 11, fontWeight: 700, color: "#B87333", marginLeft: 4 }}>
            {["", "Poor", "Fair", "Good", "Great", "Excellent"][fbStar]}
          </span>
        )}
      </div>
