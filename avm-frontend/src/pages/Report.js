...
// ✅ NEW (ADDED ONLY): create small deterministic signature for report cache
...
export default function Report() {
  ...
  // ✅ NEW (ADDED ONLY): hide header when logged in
  ...
  // ✅ requirement #5: show saved report on reload (if same inputs), otherwise fetch
  useEffect(() => {
    const meta = safeParse(localStorage.getItem(LS_REPORT_META)) || {};
    const storedReport = safeParse(localStorage.getItem(LS_REPORT_KEY));

    const currentHash = hashLike(stableStringify(formData));
    const savedHash = meta?.formHash;

    if (storedReport && savedHash && savedHash === currentHash) {
      setReportData(storedReport);
      setLoading(false);
      setErr("");
      // still allow DB update if missing (handled below) but no re-fetch needed
    } else {
      setLoading(true);
    }
  }, [formData]);
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        // ✅ if already have cached report for same input, skip API fetch
        if (storedReport && savedHash && savedHash === currentHash) {
          // still update DB once if needed
          if (!savedRef.current) {
            const valuationRowId = localStorage.getItem(LS_VAL_ROW_ID);
            const est = Number(storedReport?.total_valuation);
            ...
            if (valuationRowId && Number.isFinite(est)) {
              ...
              const { error: upErr } = await supabase
                ...
                .update({
                  estimated_valuation: est,
                  report_payload: storedReport,
                  updated_at: new Date().toISOString(),
                })
                .eq("id", valuationRowId);

              if (upErr) {
                console.error("Failed to update valuation row (cached path):", upErr);
                savedRef.current = false;
              }
            }
          }
        ...
        // ✅ store report + confirm hash so reload shows it
        localStorage.setItem(LS_REPORT_KEY, JSON.stringify(json));
        localStorage.setItem(LS_REPORT_META, JSON.stringify({ formHash: currentHash }));

        // ✅ requirement #4: update valuations row with report + estimated valuation
        if (!savedRef.current) {
          const valuationRowId = localStorage.getItem(LS_VAL_ROW_ID);
          const est = Number(json?.total_valuation);
    ...
    // if no formData, do nothing
    if (formData && Object.keys(formData).length > 0) run();
  ...
  return (
    <div className="reportPage">
      {/* ✅ requirement #2: hide header when logged in */}
