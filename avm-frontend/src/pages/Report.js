...
//           </div>
//           <nav className="acq-legal">
//             {["Legal links", "Terms", "Privacy", "Cookies", "Security"].map((l) => (<a key={l} href="#">{l}</a>))}
//           </nav>
//         </div>
//       </footer>
//     </>
...
// ============================================================
//  ACQAR — Report.js  (FIXED — Share Link Bugs 1-5)
//  Fix summary:
//  BUG 1 — form_payload + report_payload saved on UPDATE call
//  BUG 2 — lsValRowId state initialised from localStorage on mount
//  BUG 3 — loadValuation handles unauthenticated share viewers
//           (falls back to report_payload so no re-fetch needed)
//  BUG 4 — shareValId regex supports both int8 AND uuid IDs
//  BUG 5 — share link input shows immediately after save,
//           not stuck waiting on a stale loading flag
// ============================================================
...
// ─── BUG 4 FIX: accept both integer ids and UUIDs ────────────
  ...
  return (
    <>
      <style>{`
        ...
        .acq-brand-desc { font-size:.75rem; color:rgba(43,43,43,.58); line-height:1.75; margin:0 0 18px; max-width:240px; }
        .acq-rics-badge { display:inline-flex; align-items:center; gap:7px; padding:7px 12px; background:#fff; border:1px solid #EBEBEB; border-radius:8px; margin-bottom:20px; }
        .acq-rics-badge span { font-size:.5625rem; font-weight:800; color:rgba(43,43,43,.7); text-transform:uppercase; letter-spacing:.1em; }
        .acq-col-title { font-size:.5625rem; font-weight:800; letter-spacing:.18em; text-transform:uppercase; color:rgba(43,43,43,.38); margin:0 0 16px; }
        .acq-link-list { list-style:none; margin:0; padding:0; display:flex; flex-direction:column; gap:10px; }
        .acq-link-item { font-size:.75rem; font-weight:500; color:rgba(43,43,43,.6); cursor:pointer; transition:color .16s; }
        .acq-link-item:hover { color:#2B2B2B; }
        .acq-divider { max-width:80rem; margin:0 auto; padding:0 2rem; }
        .acq-divider hr { border:none; border-top:1px solid #EBEBEB; margin:0; }
        .acq-footer-bottom { max-width:80rem; margin:0 auto; padding:18px 2rem 28px; display:flex; justify-content:space-between; align-items:center; }
        .acq-copy p { font-size:.5625rem; font-weight:700; color:rgba(43,43,43,.35); text-transform:uppercase; letter-spacing:.14em; margin:0; }
        .acq-legal { display:flex; align-items:center; gap:28px; flex-wrap:wrap; justify-content:flex-end; }
        .acq-legal a { font-size:.5625rem; font-weight:800; color:rgba(43,43,43,.38); text-transform:uppercase; letter-spacing:.12em; text-decoration:none; white-space:nowrap; transition:color .16s; }
        .acq-legal a:hover { color:#2B2B2B; }
        @media(max-width:1024px){ .acq-footer-grid{ grid-template-columns:1fr 1fr 1fr; gap:32px; } .acq-brand-col{ grid-column:1/-1; } }
        @media(max-width:640px){ .acq-footer-grid{ grid-template-columns:1fr 1fr; gap:28px; padding:0 1rem 40px; } .acq-brand-col{ grid-column:1/-1; } .acq-footer-bottom{ flex-direction:column; align-items:center; text-align:center; gap:14px; padding:18px 1rem 28px; } .acq-legal{ justify-content:center; gap:18px; } }
        @media(max-width:420px){ .acq-footer-grid{ grid-template-columns:1fr; } }
      `}</style>
      <footer className="acq-footer">
        <div className="acq-footer-grid">
        ...
        <div className="acq-footer-bottom">
          ...
          <nav className="acq-legal">
            {["Terms", "Privacy", "Cookies", "Security"].map((l) => (<a key={l} href="#" onClick={(e) => { e.preventDefault(); navigate(`/${l.toLowerCase()}`); }}>{l}</a>))}
          </nav>
        </div>
      </footer>
    </>
...
export default function Report() {
  const navigate     = useNavigate();
  const [sp]         = useSearchParams();
  const valuationId  = sp.get("id"); // present when opened via share link
  const location     = useLocation();
  ...
  // ── BUG 2 FIX: initialise from localStorage immediately so shareUrl ──
  //    is available on first render without waiting for the API call.
  const [lsValRowId, setLsValRowId] = useState(
    () => localStorage.getItem(LS_VAL_ROW_ID) || ""
  );
  ...
  // ── Load form data from localStorage (normal flow) ────────
  useEffect(() => {
    if (valuationId) return; // share-link mode handled separately
    const storedForm = safeParse(localStorage.getItem(LS_FORM_KEY));
    if (storedForm) setFormData(storedForm);
  }, [valuationId]);

  // ── BUG 3 FIX: share-link mode — load row from Supabase ───
  //    Works for unauthenticated viewers because the RLS policy
  //    must allow: SELECT on valuations FOR ALL (or by id).
  //    Add this policy in Supabase if not present:
  //      CREATE POLICY "public_read_by_id" ON valuations
  ...
  useEffect(() => {
    ...
    async function loadValuation() {
      try {
        ...
        // BUG 4 FIX: validate id — accept int8 OR uuid
        const cleanId = String(valuationId).trim();
        if (!isValidId(cleanId)) {
          if (!mounted) return;
          setErr("Invalid share link — the report ID is malformed.");
          setLoading(false);
          return;
        }

        // Cast to number only if it's an integer id
        const idParam = /^\d+$/.test(cleanId) ? Number(cleanId) : cleanId;
        ...
        if (!data) {
          if (!mounted) return;
          setErr("This shared report was not found. It may have been deleted.");
          setLoading(false);
          return;
        }
        ...
        // BUG 3 FIX: try report_payload first (pre-computed, no API call needed)
        const cachedReport = data?.report_payload
          ? (typeof data.report_payload === "string" ? safeParse(data.report_payload) : data.report_payload)
          : null;

        if (cachedReport && typeof cachedReport === "object") {
          setReportData(cachedReport);
          setLoading(false);
          return; // ✅ done — no need to hit the API
        }

        // Fallback: load form_payload and re-run prediction
        const payload = data?.form_payload || data?.payload || null;
        const obj = typeof payload === "string" ? safeParse(payload) : payload;

        if (obj && typeof obj === "object") {
          setFormData(obj);
          // The main API useEffect will fire once formData is set
        } else {
          setErr("This shared report could not be loaded (missing payload). Please ask the owner to regenerate it.");
          setLoading(false);
        }
      } catch (e) {
        if (!mounted) return;
        setErr(e?.message || "Failed to load shared report.");
        setLoading(false);
      }
    }
  ...
  // ── Main API call — runs in both normal & share-link mode ──
  useEffect(() => {
    // Skip if we already have reportData from cached report_payload
    if (reportData && valuationId) return;
    ...
    async function run() {
      try {
        ...
        if (!API) throw new Error("REACT_APP_AVM_API is missing. Set it in .env and restart.");

        // Share-link mode: wait for formData to be populated from Supabase
        if (valuationId && (!formData || Object.keys(formData).length === 0)) return;

        // Normal mode: formData must exist
        if (!valuationId && (!formData || Object.keys(formData).length === 0)) {
          throw new Error("No form data found. Please fill in the valuation form first.");
        }
        ...
        // Cache in localStorage for normal (non-share) flow
        if (!valuationId) localStorage.setItem(LS_REPORT_KEY, JSON.stringify(merged));

        // ── BUG 1 FIX: save form_payload + report_payload so share links work ──
        if (!savedRef.current) {
          const rowId = localStorage.getItem(LS_VAL_ROW_ID);
          const est   = Number(merged?.total_valuation);

          if (rowId && Number.isFinite(est)) {
            savedRef.current = true;
            ...
            const { error: upErr } = await supabase
              ...
              .update({
                estimated_valuation: est,
                updated_at:          new Date().toISOString(),
                // ✅ These two fields power the share link
                form_payload:        formData,
                report_payload:      JSON.stringify(merged),
              })
              .eq("id", rowId);
            ...
            if (upErr) {
              console.error("Failed to persist valuation payload:", upErr);
              savedRef.current = false; // allow retry on next render
            } else {
              // ── BUG 2 FIX: set state so shareUrl renders immediately ──
              setLsValRowId(String(rowId));
            }
          }
        }
      } catch (e) {
        if (!mounted) return;
        setErr(e?.message || "Something went wrong generating the report.");
      } finally {
        if (!mounted) return;
        setLoading(false);
      }
  ...
  // BUG 2 + 4 FIX: id resolved from URL param → valRow → lsValRowId state
  //                (state is pre-seeded from localStorage on mount)
  const shareValId = useMemo(() => {
    const candidates = [
      valuationId,
      valRow?.id != null ? String(valRow.id) : "",
  ...
  // BUG 5 FIX: shareUrl is derived from shareValId which updates as soon
  //            as the Supabase update succeeds — no stale loading dependency.
  const shareUrl = shareValId
    ? `${window.location.origin}/report?id=${encodeURIComponent(shareValId)}`
    : "";
  ...
  async function handleCopyShareLink() {
    if (!shareUrl) {
      alert("Report is still saving. Please wait a moment and try again.");
      return;
    }
    try {
      await navigator.clipboard.writeText(shareUrl);
    ...
    setCopied(true);
    window.clearTimeout(handleCopyShareLink._t);
    handleCopyShareLink._t = window.setTimeout(() => setCopied(false), 1800);
  }
  ...
  async function submitFeedback(rating, note) {
    ...
    try {
      ...
      const valId = shareValId && isValidId(shareValId)
        ? (/^\d+$/.test(shareValId) ? Number(shareValId) : shareValId)
        : null;
      const payload = {
        rating,
  ...
  const forecastSeries = useMemo(() => {
    const hist = (reportData?.forecast?.historical || []).slice(-12).map((r) => ({
      label: monthLabel(r.month), psm: r.median_price_per_sqm, is_forecast: false,
    }));
    const proj = (reportData?.forecast?.forecast || []).map((r) => ({
      label: monthLabel(r.month), psm: r.median_price_per_sqm, is_forecast: true,
    }));
    return [...hist, ...proj];
  }, [reportData]);

  const supplyDemandSeries = useMemo(() => (
    (reportData?.supply_demand?.monthly || []).slice(-18).map((r) => ({
      label: monthLabel(r.month), transactions: r.transactions,
    }))
  ), [reportData]);
  ...
  const filteredComparables = useMemo(() => {
    ...
    return list
      .filter((c) => {
        if (!subjectProp) return true;
        const compProp = String(c?.project_name_en || c?.building_name_en || c?.master_project_en || "").trim().toLowerCase();
        return !compProp || compProp !== subjectProp;
      })
      .sort((a, b) => (Number(b?.match_pct) || 0) - (Number(a?.match_pct) || 0));
  }, [reportData, formData]);
  ...
  const displayUserName = useMemo(() => {
    ...
    if (loggedUser.user_metadata?.full_name) return loggedUser.user_metadata.full_name;
    if (loggedUser.email) {
      const n = loggedUser.email.split("@")[0];
  ...
  const factorWeights = useMemo(() => {
    const level = reportData?.tx?.anchor_level || "area";
    const comps = Number(reportData?.comparables_meta?.count || 0);
    const locationW = level === "project" ? 40 : level === "master_project" ? 35 : 25;
    const dataW     = Math.min(comps * 2, 20);
    const remaining = 100 - locationW - dataW;
    return [
      { name: "Location",      value: locationW },
      { name: "Market Data",   value: dataW     },
      { name: "Property Type", value: Math.round(remaining * 0.28) },
      { name: "Size & Floor",  value: Math.round(remaining * 0.24) },
      { name: "Condition",     value: Math.round(remaining * 0.26) },
      { name: "Amenities",     value: Math.round(remaining * 0.22) },
    ];
  }, [reportData]);

  // ── Core metrics ──────────────────────────────────────────
  const areaName    = formData?.area_name_en || formData?.district_name || "—";
  const subArea     = formData?.sub_area_en  || formData?.community_en  || "";
  const projectName = formData?.project_name_en || formData?.building_name || formData?.building_name_en || "—";
  ...
  const trusteeFee     = 4000;
  const mortgageRegFee = loanAmount ? loanAmount * 0.0025 : null;
  const totalBuyingCost = dldFee && agentBuyFee && mortgageRegFee
    ? dldFee + agentBuyFee + trusteeFee + mortgageRegFee : null;
  ...
  // ── CSS ───────────────────────────────────────────────────
  const CSS = `
    :root { --acq-accent:#B87333; --acq-primary:#2B2B2B; }
    .reportPage { min-height:100vh; background:#F8F8F8; font-family:'Inter',system-ui,sans-serif; color:#2B2B2B; }
    .acqHdrLite { position:fixed; top:0; left:0; right:0; z-index:100; height:64px; background:rgba(255,255,255,.95); border-bottom:1px solid #EBEBEB; backdrop-filter:blur(8px); display:flex; align-items:center; }
    .acqHdrLiteInner { max-width:80rem; margin:0 auto; padding:0 2rem; width:100%; }
    .acqHdrLogo { cursor:pointer; display:inline-block; }
    .acqHdrLiteSpacer { height:64px; }
    .vcMain { max-width:900px; margin:0 auto; padding:32px 24px 80px; }
    .vcHeader { padding:28px 0 20px; border-bottom:1px solid #F0F0F0; }
    .vcTitle { font-size:28px; font-weight:800; letter-spacing:-.02em; margin:0 0 10px; color:#2B2B2B; }
    .vcMeta { display:flex; flex-wrap:wrap; gap:6px; align-items:center; font-size:13px; color:rgba(43,43,43,.6); margin-bottom:10px; }
    .vcDot { width:4px; height:4px; border-radius:50%; background:rgba(43,43,43,.25); display:inline-block; }
    .vcHeaderRow { display:flex; gap:24px; margin-top:8px; }
    .vcMini { display:flex; gap:8px; font-size:11px; }
    .vcMini span:first-child { color:rgba(43,43,43,.4); font-weight:600; text-transform:uppercase; letter-spacing:.06em; }
    .vcMini span:last-child  { font-weight:600; color:#2B2B2B; font-family:ui-monospace,monospace; }
    .vcSectionGrid { display:grid; grid-template-columns:1fr 1fr; gap:32px; margin-top:32px; padding-top:32px; border-top:1px solid #F0F0F0; }
    .vcSmallTitle { font-size:10px; font-weight:600; letter-spacing:.1em; text-transform:uppercase; color:rgba(43,43,43,.4); margin:0 0 16px; }
    .vcValueBig { font-size:48px; font-weight:700; letter-spacing:-.02em; margin:0; color:#2B2B2B; }
    .vcValueSub { font-size:13px; color:rgba(43,43,43,.5); font-weight:400; margin-top:6px; }
    .vcBar { height:8px; background:#F5F5F5; border-radius:4px; overflow:hidden; display:flex; margin-top:20px; }
    .vcBar>div { height:100%; }
    .vcBarLow  { width:25%; background:#E5E5E5; }
    ...
    .vcUnlockBtn { border:none; background:transparent; color:#B87333; font-weight:700; font-size:11px; letter-spacing:.05em; text-transform:uppercase; border-bottom:1.5px solid #B87333; padding:0 0 4px; cursor:pointer; }
    .vcCards { display:grid; grid-template-columns:repeat(3,1fr); gap:16px; }
    .vcCard  { border:1px solid #E8E8E8; padding:16px; border-radius:8px; background:#FFFFFF; }
    .vcTagRow { display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; }
    .vcTag { font-size:9px; font-weight:800; letter-spacing:.08em; text-transform:uppercase; padding:3px 8px; border-radius:4px; background:#F0F7FF; color:#1d4ed8; }
    .vcCardTitle { font-size:13px; font-weight:700; margin:0 0 6px; color:#2B2B2B; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .vcCardMeta  { font-size:11px; color:rgba(43,43,43,.5); margin:0 0 12px; }
    .vcCardPrice { font-size:18px; font-weight:700; color:#2B2B2B; margin:0; font-family:ui-monospace,monospace; }
    .vcCardPsm   { font-size:11px; color:rgba(43,43,43,.5); margin:2px 0 0; }
    .vcMacroSection { margin-top:48px; }
    .vcMacroGrid { display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-top:16px; }
    .vcMacroCard { border:1px solid #E8E8E8; border-radius:8px; padding:16px; background:#fff; }
    .vcMacroLabel { font-size:10px; font-weight:600; text-transform:uppercase; letter-spacing:.08em; color:rgba(43,43,43,.4); margin:0 0 8px; }
    .vcMacroValue { font-size:22px; font-weight:700; color:#2B2B2B; margin:0; }
    .vcMacroSub   { font-size:11px; color:rgba(43,43,43,.45); margin:4px 0 0; }
    .vcFbSection  { margin-top:48px; border:1px solid #E8E8E8; border-radius:12px; padding:24px 28px; background:#fff; }
    .vcFbHead     { font-size:14px; font-weight:700; margin:0 0 20px; color:#2B2B2B; }
    .vcFbTopRow   { display:flex; gap:16px; align-items:flex-start; margin-bottom:20px; }
    .vcFbLeft     { flex:1; min-width:0; }
    .vcFbChoice   { flex:1; padding:14px; border:2px solid #E8E8E8; border-radius:8px; cursor:pointer; background:#fff; font-size:13px; font-weight:600; color:#2B2B2B; transition:all .15s; }
    .vcFbChoice:hover   { border-color:#B87333; }
    .vcFbChoice.active  { border-color:#B87333; background:#FFF8F3; }
    .vcFbRight    { width:200px; flex-shrink:0; }
    .vcFbNote     { width:100%; border:1px solid #E8E8E8; border-radius:6px; padding:10px 12px; font-size:13px; font-family:inherit; resize:vertical; min-height:72px; }
    .vcFbActions  { display:flex; gap:12px; flex-wrap:wrap; }
    .vcFbSubmit   { padding:12px 24px; background:#B87333; color:#fff; border:none; border-radius:6px; font-weight:700; font-size:12px; text-transform:uppercase; letter-spacing:.08em; cursor:pointer; min-width:220px; }
    .vcFbSubmit:disabled { opacity:.45; cursor:not-allowed; }
    .vcFbBack     { padding:12px 18px; background:transparent; border:1px solid #E8E8E8; border-radius:6px; font-weight:600; font-size:12px; cursor:pointer; color:#2B2B2B; }
    .vcRewardScreen { text-align:center; padding:20px 0; }
    .vcRewardCheck  { width:40px; height:40px; background:#F0FDF4; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 12px; }
    .vcRewardTitle  { font-size:18px; font-weight:800; margin:0 0 6px; }
    .vcRewardSub    { font-size:13px; color:rgba(43,43,43,.6); margin:0 0 16px; }
    .vcVoucher      { display:flex; justify-content:space-between; align-items:center; border:1.5px dashed #B87333; border-radius:10px; padding:14px 18px; background:#FFF8F3; margin-bottom:12px; }
    .vcVoucherLeft  { display:flex; align-items:center; gap:10px; }
    .vcVoucherIcon  { font-size:22px; }
    .vcVoucherCode  { font-size:10px; font-weight:800; letter-spacing:.1em; text-transform:uppercase; color:#B87333; }
    .vcVoucherName  { font-size:12px; font-weight:700; color:#2B2B2B; }
    .vcApplyBtn     { padding:8px 16px; background:#B87333; color:#fff; border:none; border-radius:6px; font-weight:700; font-size:11px; cursor:pointer; letter-spacing:.06em; text-transform:uppercase; }
    .vcBottomSection  { margin-top:48px; border:1px solid #E8E8E8; border-radius:12px; padding:24px 28px; background:#fff; }
    .vcShareSection   { margin-bottom:24px; }
    .vcShareLabel     { font-size:10px; font-weight:700; letter-spacing:.12em; text-transform:uppercase; color:rgba(43,43,43,.4); margin:0 0 10px; }
    .vcShareRow       { display:flex; gap:8px; }
    .vcShareInput     { flex:1; border:1px solid #E8E8E8; border-radius:6px; padding:10px 12px; font-size:12px; font-family:ui-monospace,monospace; background:#FAFAFA; color:#2B2B2B; outline:none; }
    .vcShareInput:focus { border-color:#B87333; }
    .vcCopyBtn        { padding:10px 20px; background:#2B2B2B; color:#fff; border:none; border-radius:6px; font-weight:700; font-size:11px; cursor:pointer; text-transform:uppercase; letter-spacing:.06em; white-space:nowrap; transition:background .15s; }
    .vcCopyBtn:hover:not(:disabled) { background:#000; }
    .vcCopyBtn:disabled { opacity:.4; cursor:not-allowed; }
    .vcFooterInfo     { display:grid; grid-template-columns:1fr 1fr 1fr; gap:24px; padding-top:20px; border-top:1px solid #F0F0F0; }
    .vcInfoTitle      { font-size:9px; font-weight:800; letter-spacing:.1em; text-transform:uppercase; color:rgba(43,43,43,.38); margin:0 0 6px; }
    .vcInfoContent    { font-size:12px; color:rgba(43,43,43,.65); margin:0; line-height:1.55; }
    .vcInfoList       { font-size:12px; color:rgba(43,43,43,.65); margin:6px 0 0; padding-left:16px; line-height:1.8; }
    .vcActions        { display:flex; gap:12px; margin-top:24px; padding-top:20px; border-top:1px solid #F0F0F0; }
    .vcBtn            { padding:10px 20px; border-radius:6px; font-weight:700; font-size:11px; cursor:pointer; letter-spacing:.06em; text-transform:uppercase; border:none; }
    .vcBtnGhost       { background:transparent; border:1px solid #E8E8E8; color:#2B2B2B; }
    .vcBtnPrimary     { background:#2B2B2B; color:#fff; }
    .finGrid  { display:grid; grid-template-columns:repeat(3,1fr); gap:16px; margin-bottom:24px; }
    .finBox   { border:1px solid #E8E8E8; border-radius:10px; padding:18px; text-align:center; }
    .finLabel { font-size:9px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; color:rgba(43,43,43,.4); margin:0 0 8px; }
    .finValue { font-size:20px; font-weight:700; color:#2B2B2B; margin:0; }
    .finSub   { font-size:11px; color:rgba(43,43,43,.5); margin:4px 0 0; }
    .costRow  { display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid #F5F5F5; font-size:13px; }
    ...
    .featureItem { display:flex; align-items:center; gap:8px; font-size:13px; padding:10px 12px; border:1px solid #E8E8E8; border-radius:6px; background:#fff; }
    @media(max-width:1024px){ .vcSectionGrid{ grid-template-columns:1fr; } .vcCards{ grid-template-columns:1fr 1fr; } .featureGrid{ grid-template-columns:repeat(2,1fr); } .vcFooterInfo{ grid-template-columns:1fr; } .vcMacroGrid{ grid-template-columns:1fr 1fr; } .finGrid{ grid-template-columns:1fr 1fr; } }
    @media(max-width:640px){ .vcValueBig{ font-size:36px; } .vcCards{ grid-template-columns:1fr; } .featureGrid{ grid-template-columns:1fr 1fr; } .vcTitle{ font-size:22px; } .vcFbSubmit{ width:100%; min-width:0; } .vcFbLeft{ min-width:0; } .vcFbTopRow{ flex-direction:column; } .vcFbRight{ width:100%; } .vcFbChoice{ flex:1; } .finGrid{ grid-template-columns:1fr; } .vcShareRow{ flex-direction:column; } }
  `;
  ...
  return (
    <div className="reportPage">
      ...
      {copied && (
        <div
          ...
          style={{
            ...
            letterSpacing: ".04em", boxShadow: "0 10px 30px rgba(0,0,0,.18)",
          }}
        >
          ✅ Link copied!
        </div>
      ...
      <main className="vcMain">
        ...
        {/* ── Loading state ── */}
        {loading ? (
          <div style={{ marginTop: 32, background: "#fff", border: "1px solid #E8E8E8", padding: 28, borderRadius: 10 }}>
            <div style={{ fontWeight: 700, marginBottom: 8 }}>⏳ Generating your report…</div>
            <div style={{ color: "rgba(43,43,43,.55)", fontSize: 13 }}>Running AI valuation and fetching comparable transactions.</div>
          </div>

        ) : err ? (
          <div style={{ marginTop: 32, background: "#fff", border: "1px solid #FFCCCC", padding: 28, borderRadius: 10 }}>
            <div style={{ fontWeight: 700, marginBottom: 8, color: "#c00" }}>⚠ Error</div>
            <div style={{ color: "rgba(43,43,43,.7)", fontSize: 13 }}>{err}</div>
            <button onClick={goBack} style={{ marginTop: 16, padding: "10px 20px", background: "#2B2B2B", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 700, fontSize: 12 }}>
          ...
          <>
            {/* ── 1. Estimated Value + Trend chart ── */}
            <section className="vcSectionGrid" style={{ marginTop: 32, paddingTop: 32, borderTop: "1px solid #F0F0F0" }}>
              <div>
                <h2 className="vcSmallTitle">Estimated Market Value</h2>
                <p className="vcValueBig">{fmtAED(reportData?.total_valuation)}</p>
                <div className="vcValueSub">± {fmtPct(confidencePct, 0)} Confidence · {anchorLevel} level</div>
                <div className="vcBar">
                  <div className="vcBarLow" />
                  <div className="vcBarMid" />
                  <div className="vcBarHigh" />
                ...
                <div className="vcTip">
                  <p>Accuracy based on {compsCount} comparable transactions in {areaName}. Upgrade to <strong>DealLens™</strong> for ±5% institutional precision.</p>
                </div>
                {Number.isFinite(rateSqft) && (
                  <p style={{ marginTop: 12, fontSize: 13, color: "rgba(43,43,43,.5)" }}>
                    AED {fmtNum(rateSqft, 0)}/sqft · AED {fmtNum(rateSqm, 0)}/sqm
                  </p>
                )}
              </div>

              <div>
                <h2 className="vcSmallTitle" style={{ marginBottom: 12 }}>Price Trend — {areaName}</h2>
                <div className="vcChartCard">
                  {trendSeries.length >= 2 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={trendSeries}>
                        ...
                        <YAxis tickFormatter={(v) => `${(v / 1e6).toFixed(1)}M`} tick={{ fontSize: 9 }} tickLine={false} axisLine={false} width={45} />
                        <Tooltip formatter={(v) => fmtAED(v)} />
                        <Area type="monotone" dataKey="market_total"   stroke="#E8E8E8" fill="#F5F5F5" strokeWidth={1.5} name="Market Avg" />
                        <Area type="monotone" dataKey="property_total" stroke="#B87333" fill="rgba(184,115,51,.12)" strokeWidth={2} name="This Property" />
                      </AreaChart>
                    </ResponsiveContainer>
                  ) : (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "rgba(43,43,43,.35)", fontSize: 12 }}>
                      Trend data not available for this area yet.
                    </div>
                  )}
                </div>
              </div>
            ...
            <section className="vcMacroSection">
              ...
              <div className="vcMacroGrid">
                {[
                  { label: "Avg. Price / SQFT", value: Number.isFinite(rateSqft) ? `AED ${fmtNum(rateSqft, 0)}` : "—", sub: "Area benchmark" },
                  { label: "Data Quality",      value: `${dataQuality}%`,  sub: `${anchorLevel} level match` },
                  { label: "Comparables Used",  value: compsCount || "—",  sub: "Verified DLD transactions" },
                  { label: "Confidence Score",  value: `${fmtPct(confidencePct, 0)}`, sub: "AI prediction confidence" },
                ].map((m) => (
                  <div className="vcMacroCard" key={m.label}>
                    <p className="vcMacroLabel">{m.label}</p>
                    <p className="vcMacroValue">{m.value}</p>
                    <p className="vcMacroSub">{m.sub}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section style={{ marginTop: 48 }}>
              <div className="vcCardsHead">
                <div className="vcCardsTitle">
                  <div className="vcCardsSubtitle">Comparable Transactions</div>
                  <h3 className="vcCardsMainTitle">Similar Properties Sold</h3>
                </div>
                <button className="vcUnlockBtn" type="button">Unlock Full Set</button>
              </div>

              {filteredComparables.length > 0 ? (
                <div className="vcCards">
                  {filteredComparables.slice(0, 6).map((c, idx) => {
                    const title   = c?.building_name_en || c?.project_name_en || c?.master_project_en || c?.area_name_en || "Comparable";
                    const match   = Number(c?.match_pct);
                    const price   = Number(c?.price_aed  || c?.meter_sale_price || c?.actual_worth);
                    const psm     = Number(c?.price_per_sqm || c?.meter_sale_price);
                    const soldDate = fmtDate(c?.sold_date || c?.instance_date);
                    return (
                      <div className="vcCard" key={c?.transaction_id || idx}>
                        <div className="vcTagRow">
                          <span className="vcTag">{Number.isFinite(match) ? `${match.toFixed(0)}% match` : "Comparable"}</span>
                          <span style={{ fontSize: 9, color: "rgba(43,43,43,.4)", fontWeight: 600 }}>{soldDate}</span>
                        </div>
                        <p className="vcCardTitle">{title}</p>
                        <p className="vcCardMeta">{c?.area_name_en || "Dubai"} · {c?.rooms_en ? `${c.rooms_en} BR` : ""}</p>
                        <p className="vcCardPrice">{Number.isFinite(price) ? fmtAED(price) : "—"}</p>
                        {Number.isFinite(psm) && <p className="vcCardPsm">AED {fmtNum(psm, 0)}/sqm</p>}
                      </div>
                    );
                  })}
                </div>
              ) : (
                <div style={{ padding: "28px 0", textAlign: "center", color: "rgba(43,43,43,.4)", fontSize: 13, border: "1px dashed #E8E8E8", borderRadius: 8 }}>
                  No comparable transactions found for this property. <br />
                  <span style={{ fontSize: 11 }}>Try DealLens™ for a broader search radius.</span>
                </div>
              )}
            </section>
            ...
            {/* ── 4. Forecast chart ── */}
            {forecastSeries.length >= 2 && (
              <section style={{ marginTop: 48, background: "#fff", border: "1px solid #E8E8E8", borderRadius: 12, padding: "24px 28px" }}>
                <div className="vcCardsSubtitle">Price Forecast</div>
                <h3 className="vcCardsMainTitle" style={{ margin: "4px 0 20px" }}>3-Year Price Prediction</h3>
                <div style={{ height: 240 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={forecastSeries}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#F0F0F0" />
                      <XAxis dataKey="label" tick={{ fontSize: 9 }} tickLine={false} axisLine={false} />
                      <YAxis tickFormatter={(v) => `${fmtNum(v, 0)}`} tick={{ fontSize: 9 }} tickLine={false} axisLine={false} width={55} />
                      <Tooltip formatter={(v) => [`AED ${fmtNum(v, 0)}/sqm`, "Price/sqm"]} />
                      <Line type="monotone" dataKey="psm" stroke="#B87333" strokeWidth={2} dot={false} strokeDasharray={(d) => d?.is_forecast ? "4 4" : "0"} name="Price/sqm" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </section>
            ...
            {/* ── 5. Supply & demand ── */}
            {supplyDemandSeries.length >= 2 && (
              <section style={{ marginTop: 32, background: "#fff", border: "1px solid #E8E8E8", borderRadius: 12, padding: "24px 28px" }}>
                <div className="vcCardsSubtitle">Market Activity</div>
                <h3 className="vcCardsMainTitle" style={{ margin: "4px 0 20px" }}>Monthly Transaction Volume</h3>
                <div style={{ height: 200 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={supplyDemandSeries}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#F0F0F0" />
                      <XAxis dataKey="label" tick={{ fontSize: 9 }} tickLine={false} axisLine={false} />
                      <YAxis tick={{ fontSize: 9 }} tickLine={false} axisLine={false} width={30} />
                      <Tooltip />
                      <Bar dataKey="transactions" fill="#B87333" radius={[3, 3, 0, 0]} name="Transactions" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </section>
            ...
            {/* ── 6. Valuation factors (Pie) ── */}
            <section style={{ marginTop: 32, background: "#fff", border: "1px solid #E8E8E8", borderRadius: 12, padding: "24px 28px" }}>
              <div className="vcCardsSubtitle">AI Valuation Breakdown</div>
              <h3 className="vcCardsMainTitle" style={{ margin: "4px 0 20px" }}>Valuation Factor Weights</h3>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, alignItems: "center" }}>
                <div style={{ height: 220 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={factorWeights} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, value }) => `${name} ${value}%`} labelLine={false}>
                        {factorWeights.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                      </Pie>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {factorWeights.map((f, i) => (
                    <div key={f.name} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13 }}>
                      <div style={{ width: 10, height: 10, borderRadius: "50%", background: PIE_COLORS[i % PIE_COLORS.length], flexShrink: 0 }} />
                      <span style={{ flex: 1, color: "#2B2B2B" }}>{f.name}</span>
                      <span style={{ fontWeight: 700, fontFamily: "ui-monospace,monospace", color: "#2B2B2B" }}>{f.value}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {Number.isFinite(totalVal) && (
              <section style={{ marginTop: 32, background: "#fff", border: "1px solid #E8E8E8", borderRadius: 12, padding: "24px 28px" }}>
                <div className="vcCardsSubtitle">Financial Analysis</div>
                <h3 className="vcCardsMainTitle" style={{ margin: "4px 0 20px" }}>Investment Snapshot</h3>
                <div className="finGrid">
                  {[
                    { label: "Market Value",     value: fmtAED(totalVal),       sub: "AI estimate" },
                    { label: "Down Payment (20%)",value: fmtAED(downPayment),   sub: "@ 20% LTV" },
                    { label: "Monthly Payment",  value: fmtAED(monthlyPayment), sub: `${mortgageYears}yr @ ${(mortgageRate * 100).toFixed(1)}%` },
                  ].map((b) => (
                    <div className="finBox" key={b.label}>
                      <p className="finLabel">{b.label}</p>
                      <p className="finValue">{b.value}</p>
                      <p className="finSub">{b.sub}</p>
                    </div>
                  ))}
                </div>
                <div style={{ background: "#FAFAFA", border: "1px solid #F0F0F0", borderRadius: 8, padding: "16px 20px" }}>
                  <p className="vcCardsSubtitle" style={{ marginBottom: 12 }}>Estimated Buying Costs</p>
                  {[
                    ["DLD Registration Fee (4%)",         fmtAED(dldFee)],
                    ["Agent Commission (2%)",              fmtAED(agentBuyFee)],
                    ["Trustee / Admin Fee",                fmtAED(trusteeFee)],
                    ["Mortgage Registration (0.25%)",      fmtAED(mortgageRegFee)],
                  ].map(([label, value]) => (
                    <div className="costRow" key={label}>
                      <span className="costLabel">{label}</span>
                      <span className="costValue">{value}</span>
                    </div>
                  ))}
                  <div className="costTotal">
                    <span>Total Transaction Cost</span>
                    <span>{fmtAED(totalBuyingCost)}</span>
                  </div>
                </div>
              </section>
            )}
            ...
            {/* ── 8. Feedback ── */}
            <section className="vcFbSection">
              <p className="vcFbHead">Was this valuation accurate?</p>
              {fbStep === "choose" && (
                <div className="vcFbTopRow">
                  {[
                    ...
                    <button
                      key={opt.id}
                      type="button"
                      className={`vcFbChoice${fbRating === opt.id ? " active" : ""}`}
                      onClick={() => { setFbRating(opt.id); setFbStep("form"); }}
                    >
                      {opt.label}
                    </button>
                  ))}
              ...
              {fbStep === "form" && (
                <div>
                  <textarea
                    className="vcFbNote"
                    placeholder="Optional: share any context (recent renovations, building amenities…)"
                    value={fbNote}
                    onChange={(e) => setFbNote(e.target.value)}
                  />
                  <div className="vcFbActions" style={{ marginTop: 12 }}>
                    <button className="vcFbSubmit" type="button" disabled={fbSubmitting || !fbRating} onClick={() => submitFeedback(fbRating, fbNote)}>
                      SUBMIT FEEDBACK &amp; CLAIM REWARD 🎟️
                    </button>
                    <button className="vcFbBack" type="button" disabled={fbSubmitting} onClick={() => { setFbStep("choose"); setFbRating(""); setFbNote(""); }}>
                      GO BACK
                    </button>
                  </div>
                </div>
                ...
                <div className="vcRewardScreen">
                  ...
                  <div className="vcVoucher">
                    <div className="vcVoucherLeft">
                      ...
                      <div>
                        <div className="vcVoucherCode">VOUCHER CODE: FEEDBACK100</div>
                        <div className="vcVoucherName">1× FREE DEALLENS™ REPORT</div>
                      </div>
                    </div>
                    <button className="vcApplyBtn" type="button" onClick={() => navigate("/deallens")}>APPLY NOW</button>
                  </div>
                  <button className="vcFbBack" type="button" onClick={() => { setFbStep("choose"); setFbRating(""); setFbNote(""); }} style={{ marginTop: 10 }}>
                    GO BACK
                  </button>
                </div>
              )}
            ...
            {/* ── 9. Share + Legal footer ── */}
            <section className="vcBottomSection">
              {/* ✅ SHARE LINK — all 5 bugs fixed */}
              <div className="vcShareSection">
                <p className="vcShareLabel">📤 Public Shareable Link</p>
                <div className="vcShareRow">
                  <input
                    ...
                    value={shareUrl}
                    readOnly
                    placeholder={loading ? "Saving report…" : "Generating link — please wait…"}
                  />
                  <button
                    className="vcCopyBtn"
                    onClick={handleCopyShareLink}
                    disabled={!shareUrl}
                    title={shareUrl ? "Copy shareable link" : "Saving report before link is ready…"}
                  >
                    Copy
                  </button>
                </div>
                {!shareUrl && !loading && (
                  <p style={{ margin: "8px 0 0", fontSize: 11, color: "rgba(43,43,43,.4)", fontStyle: "italic" }}>
                    The shareable link will appear once the report has been saved to your account.
                  </p>
                )}
                {shareUrl && (
                  <p style={{ margin: "8px 0 0", fontSize: 11, color: "#16a34a", fontWeight: 600 }}>
                    ✅ Link ready — anyone with this link can view this report.
                  </p>
                )}
              </div>
              ...
              <div className="vcActions">
                <button className="vcBtn vcBtnGhost"    onClick={goBack}>Regenerate Report</button>
                <button className="vcBtn vcBtnPrimary"  onClick={() => navigate("/my-reports")}>My Reports</button>
              </div>
            </section>
