...
//                     <div className="heroRangeRow">
//                       <div className="heroRangeText">
//                         Range: <b>{rangeLow ? fmtAED(rangeLow) : "—"}</b> – <b>{rangeHigh ? fmtAED(rangeHigh) : "—"}</b>
//                       </div>
//                       <span className="pill">{stabilityLabel}</span>
//                     </div>
//                   </div>
...
//                         const name = c.project_name_en || c.building_name_en || c.master_project_en || "Property";
//                         const area = c.area_name_en || "—";
//                         const rooms = normalizeRooms(c.rooms_en) || "—";
//                         const sizeSqft = Number(c.size_sqft);
...
// ✅ NEW: same color system you used elsewhere (ONLY styling)
const THEME = {
  BTN: "#b45309",
  BTN_DARK: "#92400e",
  PAGE_BG: "#FAFAFA",
...
export default function Report() {
  ...
  // ✅ ONLY colors updated (orange/black theme)
  const PIE_COLORS = [THEME.BTN, "#111827", "#D4D4D4", "#B3B3B3", "#F5F5F5", "#92400e"];
  ...
  // ✅ UI-only overrides (no functional changes)
  const pageStyle = { background: THEME.PAGE_BG, minHeight: "100vh" };
  const wrapStyle = { maxWidth: 1180, margin: "0 auto", padding: "18px 16px" };
  ...
  return (
    <div className="reportPage" style={pageStyle}>
      {!isLoggedIn ? <NavBar /> : null}

      <div className="reportWrap" style={wrapStyle}>
        <div className="topRow">
          <div>
            <div className="reportTitle" style={{ color: THEME.TEXT }}>
              Valuation Report
            </div>
            <div className="reportSub" style={{ color: THEME.MUTED }}>
              {formData?.area_name_en ? `${formData.area_name_en}` : ""}
              {formData?.property_type_en ? ` • ${formData.property_type_en}` : ""}
              {formData?.project_name_en ? ` • ${formData.project_name_en}` : ""}
              {formData?.rooms_en ? ` • ${normalizeRooms(formData.rooms_en)}` : ""}
          ...
          <div className="topActions">
            <button className="btnSecondary" style={btnSecondaryStyle} onClick={goBack}>
              Edit Inputs
            </button>
          </div>
        </div>
        ...
        {loading ? (
          <div className="card2" style={{ marginTop: 14, ...softCardStyle }}>
            <div className="card2Title" style={{ color: THEME.TEXT }}>
              Loading report…
            </div>
            <div className="card2Hint" style={{ color: THEME.MUTED }}>
              Generating prediction and fetching comparables
            </div>
          </div>
        ) : err ? (
          <div className="card2" style={{ marginTop: 14, ...softCardStyle }}>
            ...
            <div className="empty2" style={{ color: THEME.MUTED }}>
              {err}
            </div>
          </div>
        ) : (
          <>
            <div className="heroCard" style={{ marginTop: 14, ...softCardStyle }}>
              <div className="heroTop">
                <div className="heroLeft">
                  <div
                    className="heroIcon"
                  ...
                  <div className="heroMeta">
                    <div className="heroName" style={{ color: THEME.TEXT }}>
                      {projectName} <span className="heroDot">•</span>{" "}
                      {String(propertyType).toLowerCase()}
                    </div>

                    <div className="heroLoc" style={{ color: THEME.MUTED }}>
                      {areaName}
                      {subArea ? ` • ${subArea}` : ""}
                    </div>

                    <div className="heroValue" style={{ color: THEME.TEXT }}>
                      {fmtAED(reportData?.total_valuation)}
                    </div>

                    <div className="heroRangeRow">
                      <div className="heroRangeText" style={{ color: THEME.MUTED }}>
                        Range: <b style={{ color: THEME.TEXT }}>{rangeLow ? fmtAED(rangeLow) : "—"}</b> –{" "}
                        <b style={{ color: THEME.TEXT }}>{rangeHigh ? fmtAED(rangeHigh) : "—"}</b>
                      </div>
                      <span
                        className="pill"
                        style={{
                ...
                <div className="heroRight">
                  <div className="statCard" style={{ ...softCardStyle, boxShadow: "none" }}>
                    <div className="statLabel" style={{ color: THEME.MUTED }}>
                      Price/Sq.ft
                    </div>
                    <div className="statValue" style={{ color: THEME.TEXT }}>
                      {rateSqft ? fmtAED(rateSqft) : "—"}
                    </div>
                  </div>
                  ...
                  <div className="statCard statCardOk" style={{ ...softCardStyle, boxShadow: "none" }}>
                    <div className="statLabel" style={{ color: THEME.MUTED }}>
                      RICS
                    </div>
                    <div className="statValueRow">
              ...
              >
                <div
                  className="modelIcon"
                  aria-hidden="true"
                  style={{ color: THEME.BTN_DARK }}
                >
                  ⚙
                </div>
                <div className="modelText">
                  <div className="modelTitle" style={{ color: THEME.TEXT }}>
                    ML Model: {modelName}
                  </div>
                  <div className="modelSub" style={{ color: THEME.MUTED }}>
                    {modelAcc} accuracy • Updated {fmtDate(modelUpdated)}
                  </div>
                </div>
              </div>
            ...
            <div className="card2" style={{ marginTop: 14, ...softCardStyle }}>
              ...
              <div className="summaryGrid">
                <div className="summaryItem">
                  <div className="k" style={{ color: THEME.MUTED }}>
                    Total Valuation
                  </div>
                ...
                <div className="summaryItem">
                  ...
                  <div className="v" style={{ color: THEME.TEXT }}>
                    {fmtNum(reportData?.procedure_area, 2)}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="chartsRow">
              <div className="card2" style={softCardStyle}>
                <div className="card2Title" style={{ color: THEME.TEXT }}>
                  Historical Value Trend
                </div>
                <div className="card2Hint" style={{ color: THEME.MUTED }}>
                  Property Value vs Market Average
                </div>
                  ...
                  <div style={{ width: "100%", height: 280, marginTop: 10 }}>
                    <ResponsiveContainer>
                      <AreaChart data={trendSeries}>
                        ...
                        <XAxis dataKey="label" interval={5} stroke="#6B7280" />
                        <YAxis tickFormatter={(v) => fmtNum(v / 1000000, 1) + "M"} stroke="#6B7280" />
                        <Tooltip formatter={(v) => fmtAED(v)} />
                        <Area type="monotone" dataKey="market_total" fillOpacity={0.2} />
                        <Line type="monotone" dataKey="property_total" dot={false} />
                      </AreaChart>
              ...
              {comps5.length === 0 ? (
                ...
                <div className="tableWrap2">
                  <table className="compsTable">
                    <thead>
                      <tr>
                        <th style={{ color: THEME.BTN_DARK, background: THEME.SOFT2 }}>Property</th>
                        <th style={{ color: THEME.BTN_DARK, background: THEME.SOFT2 }}>Details</th>
                        <th style={{ color: THEME.BTN_DARK, background: THEME.SOFT2 }}>Price</th>
                        <th style={{ color: THEME.BTN_DARK, background: THEME.SOFT2 }}>Price/Sq.ft</th>
                        <th style={{ color: THEME.BTN_DARK, background: THEME.SOFT2 }}>Sold Date</th>
                        <th style={{ color: THEME.BTN_DARK, background: THEME.SOFT2 }}>Match</th>
                      </tr>
                    </thead>
                    <tbody>
                      {comps5.map((c, i) => {
                        ...
                        return (
                          <tr key={i}>
                            <td>
                              <div className="propTitle" style={{ color: THEME.TEXT }}>
                                {subtype} • {name}, {area}
                              </div>
                            </td>
                            <td>
                              <div className="detailsRow" style={{ color: THEME.MUTED }}>
                                <span>🛏 {rooms}</span>
                                <span style={{ marginLeft: 12 }}>📐 {sizeText}</span>
                              </div>
                            </td>
                            <td className="priceStrong" style={{ color: THEME.TEXT }}>
                              {fmtAED(c.price_aed)}
                            </td>
                            <td style={{ color: THEME.MUTED }}>{fmtAED(c.price_per_sqft)}</td>
                            <td style={{ color: THEME.MUTED }}>{fmtDate(c.sold_date)}</td>
                            <td>
                              <span
