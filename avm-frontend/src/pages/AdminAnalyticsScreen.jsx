...
// /* ─── Data ────────────────────────────────────────────────────────── */
// const visitorData = [
//   { name: 'Jan', visitors: 4500, pageViews: 12000 },
//   { name: 'Feb', visitors: 5200, pageViews: 14500 },
//   { name: 'Mar', visitors: 6100, pageViews: 18000 },
//   { name: 'Apr', visitors: 5800, pageViews: 16500 },
//   { name: 'May', visitors: 7200, pageViews: 21000 },
//   { name: 'Jun', visitors: 8500, pageViews: 25000 },
// ];

// const deviceData = [
//   { name: 'Desktop', value: 65, color: C.copper },
//   { name: 'Mobile',  value: 25, color: '#1C1C1E' },
//   { name: 'Tablet',  value: 10, color: '#C0C0C0'  },
// ];

// const locationData = [
//   { country: 'United Kingdom', visitors: '45,200', percentage: 75 },
//   { country: 'United States',  visitors: '8,400',  percentage: 12 },
//   { country: 'Germany',        visitors: '3,200',  percentage: 5  },
//   { country: 'France',         visitors: '2,100',  percentage: 3  },
//   { country: 'Others',         visitors: '3,100',  percentage: 5  },
// ];

// const stats = [
//   { label: 'Total Visitors', value: '25.4K', icon: <IcoUsers />,   trend: '+12.5%', isUp: true  },
//   { label: 'Page Views',     value: '106.2K', icon: <IcoEye />,    trend: '+18.2%', isUp: true  },
//   { label: 'Avg. Session',   value: '4m 32s', icon: <IcoClock />,  trend: '-2.4%',  isUp: false },
//   { label: 'Bounce Rate',    value: '34.2%',  icon: <IcoMouse />,  trend: '-5.1%',  isUp: true  },
// ];
...
//               <p style={{ fontSize: 14, color: C.muted, marginTop: 4 }}>Real-time traffic and engagement data for Acqar.</p>
//             </div>
//             <div className="page-header-actions" style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
//               <select style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: '9px 14px', fontSize: 13, fontWeight: 700, color: C.text, outline: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
//                 <option>Last 30 Days</option>
//                 <option>Last 90 Days</option>
//                 <option>Last Year</option>
//               </select>
//               <button style={{ display: 'flex', alignItems: 'center', gap: 7, background: C.copper, color: '#fff', border: 'none', borderRadius: 10, padding: '10px 20px', fontWeight: 800, fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer', boxShadow: '0 4px 14px rgba(200,131,42,0.25)', fontFamily: 'inherit' }}
...
const AdminAnalyticsScreen = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/analytics?range=${dateRange}`)
      ...
      .then(data => {
        ...
        const avgSession  = parseFloat(totals[2]?.value || 0);
        ...
        /* ── Traffic chart — group by month ── */
        const monthMap = {};
        rows.forEach(row => {
          const monthNum = row.dimensionValues?.[0]?.value; // "1"–"12"
          const users    = parseInt(row.metricValues?.[0]?.value || 0);
          const views    = parseInt(row.metricValues?.[1]?.value || 0);
          if (!monthNum) return;
          const key = MONTH_NAMES[parseInt(monthNum) - 1] || monthNum;
          if (!monthMap[key]) monthMap[key] = { name: key, visitors: 0, pageViews: 0 };
          monthMap[key].visitors  += users;
          monthMap[key].pageViews += views;
        });
        setVisitorData(Object.values(monthMap));
  ...
  const handleDateChange = (e) => {
    ...
    setDateRange(map[e.target.value] || '30daysAgo');
  };
      ...
      <main className="main-content" style={{ paddingTop: 60 }}>
        <div className="inner-pad" style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px 60px' }}>
          ...
          <div className="page-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 32, flexWrap: 'wrap' }}>
            ...
            <div className="page-header-actions" style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <select onChange={handleDateChange} style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: '9px 14px', fontSize: 13, fontWeight: 700, color: C.text, outline: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
                <option>Last 30 Days</option>
                <option>Last 90 Days</option>
                <option>Last Year</option>
              </select>
