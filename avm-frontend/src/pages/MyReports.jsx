...
//               .upgradeCTA .ctaDesc { font-size: 12px !important; max-width: 280px !important; margin-bottom: 22px !important; }
...
//           <div className="ctaDesc" style={{ zIndex: 1, maxWidth: 820, margin: "0 auto 28px", fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.18em", lineHeight: 1.75, color: "rgba(255,255,255,0.88)" }}>
//             UPGRADE ANY VALUCHECK™ REPORT TO DEALLENS™ FOR BANK-GRADE PRECISION AND PREDICTIVE MODELING.
//           </div>

//           <button type="button" className="ctaBtn" style={{ zIndex: 1, height: 50, padding: "0 38px", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 14, background: "linear-gradient(90deg, #B8763C 0%, #CFA24A 100%)", color: "#fff", fontSize: 10, fontWeight: 900, letterSpacing: "0.22em", textTransform: "uppercase", cursor: "pointer", boxShadow: "0 18px 46px rgba(184,115,51,0.28)", outline: "none" }}>
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        let { data: uRow } = await supabase
          .from("users")
          .select("id, role, name, email, phone, created_at, account_type, plan")
          .eq("id", authId)
          .maybeSingle();
        if (!mounted) return;
        setProfile(uRow || { id: authId, name: metaName || null, email: authEmail || null, phone: null, created_at: null });
...
>
  ...
  <button
    ...
    style={{ zIndex: 1, height: 50, padding: "0 38px", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 14, background: "linear-gradient(90deg, #B8763C 0%, #CFA24A 100%)", color: "#fff", fontSize: 10, fontWeight: 900, letterSpacing: "0.22em", textTransform: "uppercase", cursor: "pointer", boxShadow: "0 18px 46px rgba(184,115,51,0.28)", outline: "none" }}
  >
    Claim your spot →
  </button>
)}
