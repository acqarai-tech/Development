...
//   // 1. Page view
//   useEffect(() => {
//     trackEvent("page_viewed", { page: "login" });
//   }, []);
...
//       );
//       if (pct >= 25 && !scrolled[25]) { scrolled[25] = true; trackEvent("scroll_depth", { page: "login", depth: "25%" }); }
//       if (pct >= 50 && !scrolled[50]) { scrolled[50] = true; trackEvent("scroll_depth", { page: "login", depth: "50%" }); }
//       if (pct >= 75 && !scrolled[75]) { scrolled[75] = true; trackEvent("scroll_depth", { page: "login", depth: "75%" }); }
//       if (pct >= 100 && !scrolled[100]) { scrolled[100] = true; trackEvent("scroll_depth", { page: "login", depth: "100%" }); }
//     };
...
//     return () => {
//       trackEvent("time_spent", { page: "login", seconds: Math.round((Date.now() - startTime) / 1000) });
//     };
//   }, []);
...
//     e.preventDefault();
//     setMsg({ type: "", text: "" });
//     trackEvent("login_attempt", { method: otpMode ? "otp" : "email", page: "login" });
...
// // ✅ ADD THIS
// posthog.identify("admin@acqar.com", { email: "admin@acqar.com", name: "Admin" });
...
//       });
//       if (error) throw error;
//       trackEvent("login_success", { method: "email", page: "login" });
...
//   // ← CHANGE THIS:
//   sessionStorage.setItem('acqar_username', user.user_metadata?.full_name || user.user_metadata?.name || user.email)
// }
...
//  async function handleGoogleLogin() {
//     setMsg({ type: "", text: "" });
//     trackEvent("login_attempt", { method: "google", page: "login" });
//     try {
//       setOauthLoading(true);
//       const { error } = await supabase.auth.signInWithOAuth({
//         provider: "google",
//         options: {
//           redirectTo: `${window.location.origin}/auth/callback-signup`,
//           // redirectTo: `https://acqar.com/auth/callback-signup`,
//           queryParams: { prompt: "select_account" },
//         },
//       });
//       if (error) throw error;
...
//   function switchToOtp() {
//     setMsg({ type: "", text: "" });
//     trackEvent("login_method_switch", { to: "otp", page: "login" });
//     setOtpMode(true);
//     setPassword("");
//   }
...
//  function switchToPassword() {
//     setMsg({ type: "", text: "" });
//     trackEvent("login_method_switch", { to: "password", page: "login" });
//     setOtpMode(false);
//   }
...
useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session) {
        const brokerPending = sessionStorage.getItem("acqar_broker_pending");
        const pendingQuery = sessionStorage.getItem("acqar_pending_query");
        const chatPending = sessionStorage.getItem("acqar_chat_pending");
        if (brokerPending) {
