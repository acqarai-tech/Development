...
//         <div dir={rtl ? "rtl" : "ltr"} style={{ maxWidth: "75%", padding: "10px 14px", background: C.userBubble, borderRadius: "18px 18px 4px 18px", fontSize: 14, color: C.textPrimary, lineHeight: 1.6, textAlign: rtl ? "right" : "left" }}>
//           {msg.text}
//         </div>
//       </div>
//     );
...
//         <Avatar />
//         <div dir={dir} style={{ flex: 1, background: C.copperTint, border: `1px solid ${C.copperBorder}`, borderRadius: 12, padding: "16px 18px", textAlign: dir === "rtl" ? "right" : "left" }}>
//           {lines.map((line, i) => {
//             const trimmed = line.trim();
//             if (/^\d+\./.test(trimmed)) {
//               const content = trimmed.replace(/^\d+\.\s*/, "");
...
//     <div style={{ display: "flex", gap: 12, marginBottom: 28, alignItems: "flex-start" }}>
//       <Avatar />
//       <div dir={dir} style={{ flex: 1, minWidth: 0, paddingTop: 2, textAlign: dir === "rtl" ? "right" : "left" }}>
...
// // SHARED MINI COMPONENTS
// // ─────────────────────────────────────────────────────────────────
// function CardSection({ title, badge, children, dir }) {
//   return (
//     <div dir={dir} style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 10, padding: "16px 18px", marginBottom: 12, overflow: "hidden", textAlign: dir === "rtl" ? "right" : "left" }}>
//       {title && (
//         <div style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".1em", color: C.textMuted, marginBottom: 14, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
//           <span>{title}</span>
//           {badge && <span style={{ fontSize: 10, textTransform: "none", letterSpacing: 0, padding: "2px 8px", borderRadius: 4, background: C.pageBg, color: C.textMuted, fontWeight: 500 }}>{badge}</span>}
...
// function AreaMaturityCard({ msg, lang }) {
//   const intel = msg.area_intelligence || {};
//   if (!intel.area_name_en) return null;
//   const years = Object.keys(msg.price_history || {}).sort();
//   let appreciation = "—";
...
//   }
//   return (
//     <CardSection title={t("areaMaturity", lang)}>
//       <StRow label={t("yearEstablished", lang)} value={intel.year_established || "—"} />
//       <StRow label={t("masterDeveloper", lang)} value={intel.master_developer || "—"} />
//       <StRow label={t("zone", lang)} value={intel.zone_type || "—"} />
//       <StRow label={t("completionRate", lang)} value={intel.completion_rate ? `~${intel.completion_rate}%` : "—"} valueColor={C.green} />
//       <StRow label={t("residentialUnits", lang)} value={intel.residential_units ? `${intel.residential_units.toLocaleString()}` : "—"} />
//       <StRow label={t("activeOffPlanProjects", lang)} value={intel.active_project_count ? `${intel.active_project_count}` : "—"} valueColor={C.copper} />
//       <StRow label={t("fiveYearAppreciation", lang)} value={appreciation} valueColor={C.green} last />
//     </CardSection>
//   );
// }
...
// function DeveloperTrackRecordCard({ msg, lang }) {
//   const devs = msg.developer_track_records || [];
//   if (!devs.length) return null;
//   const area = msg.area_intelligence?.area_name_en || "AREA";
//   return (
//   <CardSection title={`${t("devTrackTitle", lang)} ${area.toUpperCase()}`} badge={t("historicalEstimates", lang)}>
//       <table style={{ width: "100%", borderCollapse: "collapse" }}>
//         <thead>
//           <tr>{[t("colDeveloper",lang), t("colOnTime",lang), t("colDelay",lang), t("colRating",lang), t("colSegment",lang)].map(h => (
//             <th key={h} style={{ padding: "6px 8px 6px 0", textAlign: "left", fontSize: 9, textTransform: "uppercase", color: C.textMuted, borderBottom: `1px solid ${C.border}`, fontWeight: 700 }}>{h}</th>
//           ))}</tr>
//         </thead>
//         <tbody>
...
//               <td style={{ padding: "8px 8px 8px 0", fontSize: 12, borderBottom: `1px solid ${C.border}`, fontWeight: 600 }}>{d.developer_name}</td>
//               <td style={{ padding: "8px 8px 8px 0", fontSize: 12, borderBottom: `1px solid ${C.border}`, color: d.on_time_pct >= 90 ? C.green : d.on_time_pct >= 80 ? C.amber : C.red, fontWeight: 700 }}>{d.on_time_pct}%</td>
//               <td style={{ padding: "8px 8px 8px 0", fontSize: 12, borderBottom: `1px solid ${C.border}`, color: d.avg_delay_months > 0 ? C.red : C.green }}>{d.avg_delay_months > 0 ? `~${d.avg_delay_months}` : t("onTimeOrEarly", lang)}</td>
//               <td style={{ padding: "8px 8px 8px 0", fontSize: 12, borderBottom: `1px solid ${C.border}` }}>{"★".repeat(Math.round(d.star_rating || 0))}{"☆".repeat(5 - Math.round(d.star_rating || 0))}</td>
//               <td style={{ padding: "8px 0", fontSize: 12, borderBottom: `1px solid ${C.border}`, color: C.textMuted }}>{d.market_segment}</td>
//             </tr>
//           ))}
...
// // HERO STATS ROW — matches Image 1 exactly (6 tiles)
// // ─────────────────────────────────────────────────────────────────
// function HeroStatsRow({ msg, lang }) {
//   const intel    = msg.area_intelligence || {};
//   if (!intel.area_name_en && !["buyer","seller","investor","broker"].includes(msg.user_type)) return null;
//   const stats    = msg.transaction_stats || {};
//   const userType = msg.user_type || "general";
...
//   const daysToSell = score ? Math.round(75 - parseFloat(score) * 0.4) : null;
//   const availListings = score ? Math.round(1500 + parseFloat(score) * 50) : null;
//   const moodLabel = verdict === "BUY" ? t("bullish", lang) : verdict === "HOLD" ? t("cautious", lang) : t("slow", lang);
//   const moodColor = verdict === "BUY" ? C.green : verdict === "HOLD" ? C.amber : C.red;
...
//   if (userType === "buyer") {
//     items = [
//       { lbl: t("homesSoldThisWeek", lang), val: tx ? String(tx) : (score ? String(Math.round(20 + parseFloat(score) * 1.5)) : "—"), valColor: C.red, sub: txDelta != null ? `${parseFloat(txDelta) > 0 ? "+" : ""}${txDelta}% ${t("vsLastWeek", lang)}` : t("estBasedOnScore", lang) },
//       avgPsm && { lbl: t("fairPriceHere", lang), val: `AED ${parseInt(avgPsm).toLocaleString()}/sqm`, valColor: C.textPrimary, sub: `≈ AED ${Math.round(avgPsm / 10.7639).toLocaleString()}/sqft · ${t("slightlyUp3mo", lang)}`, subColor: C.green },
//       yld && { lbl: t("rentReturnPerYear", lang), val: `${yld}%`, valColor: C.green, sub: parseFloat(yld) > 6.1 ? t("betterThanDubaiAvg", lang) : t("nearDubaiAvg", lang) },
//       daysToSell && { lbl: t("howLongToSell", lang), val: `${daysToSell} ${t("daysSuffix", lang)}`, valColor: daysToSell > 40 ? C.amber : C.green, sub: daysToSell > 40 ? t("takesLonger", lang) : t("fasterThanAvg", lang), subColor: daysToSell > 40 ? C.red : C.green },
//       availListings && { lbl: t("homesAvailable", lang), val: availListings.toLocaleString(), valColor: C.textPrimary, sub: t("moreChoice", lang) },
//       verdict && { lbl: t("marketMood", lang), val: moodLabel, valColor: moodColor, sub: verdict === "BUY" ? t("strongDemand", lang) : t("watchClosely", lang) },
//     ];
//   } else if (userType === "seller") {
//     const recPrice = firstMed ? Math.round(parseFloat(firstMed) * 1.06) : null;
//     items = [
//       avgPsm && { lbl: t("currentMarketPrice", lang), val: `AED ${parseInt(avgPsm).toLocaleString()}/sqm`, valColor: C.textPrimary, sub: t("truvaluBenchmark", lang) },
//       recPrice && { lbl: t("recommendedListPrice", lang), val: fmtAED(recPrice), valColor: C.copper, sub: `6% ${t("aboveMedian", lang)} — ${firstBr}` },
//       trend != null && { lbl: t("priceMomentum", lang), val: `${parseFloat(trend) > 0 ? "+" : ""}${trend}% YoY`, valColor: parseFloat(trend) > 0 ? C.green : C.red, sub: parseFloat(trend) > 0 ? t("risingSellStrength", lang) : t("coolingPriceCarefully", lang) },
//       tx && { lbl: t("weeklyTransactions", lang), val: String(tx), valColor: C.textPrimary, sub: txDelta != null ? `${parseFloat(txDelta) > 0 ? "+" : ""}${txDelta}% ${t("wow", lang)}` : t("dldLiveVolume", lang) },
//       distress && { lbl: t("distressSales", lang), val: `${distress}%`, valColor: parseFloat(distress) > 10 ? C.red : C.green, sub: parseFloat(distress) > 10 ? t("highPriceCompetitively", lang) : t("lowSellersLeverage", lang) },
//       verdict && { lbl: t("shouldYouSell", lang), val: trend != null && parseFloat(trend) > 0 ? t("yesGoodTime", lang) : t("hold6to12", lang), valColor: trend != null && parseFloat(trend) > 0 ? C.green : C.amber, sub: t("basedOnSignals", lang) },
//     ];
//   } else if (userType === "investor") {
//    items = [
//       yld && { lbl: t("grossYield", lang), val: `${yld}%`, valColor: parseFloat(yld) > 6.1 ? C.green : C.amber, sub: `${t("dubaiAvgLabel", lang)} · ${parseFloat(yld) > 6.1 ? t("aboveAvg4yr", lang) : t("nearAvg4yr", lang)}` },
//       distress && { lbl: t("distressOpportunity", lang), val: `${distress}%`, valColor: C.amber, sub: `${availListings ? Math.round(availListings * parseFloat(distress) / 100) : "—"} ${t("unitsPricedBelowFloor", lang)}` },
//       catScore && { lbl: t("catalystScoreLbl", lang), val: `${catScore}/100`, valColor: parseFloat(catScore) >= 70 ? C.green : C.amber, sub: `0 ${t("confirmedCatalysts", lang)}` },
//       score && { lbl: t("investmentScoreLbl", lang), val: `${score}/100`, valColor: parseFloat(score) >= 75 ? C.green : parseFloat(score) >= 60 ? C.amber : C.red, sub: parseFloat(score) >= 75 ? t("strongBuy", lang) : parseFloat(score) >= 60 ? t("buy", lang) : t("hold", lang) },
//       absRate && { lbl: t("absorptionRateLbl", lang), val: `${absRate}%`, valColor: parseFloat(absRate) > 50 ? C.green : C.amber, sub: t("fastMovingDemand", lang) },
//       trend != null && { lbl: t("capitalAppreciation", lang), val: `${parseFloat(trend) > 0 ? "+" : ""}${trend}% YoY`, valColor: parseFloat(trend) > 0 ? C.green : C.red, sub: t("priceTrendYoY", lang) },
//     ];
//   } else if (userType === "broker") {
//    items = [
//       score && { lbl: t("investmentScoreLbl", lang), val: `${score}/100`, valColor: parseFloat(score) >= 75 ? C.green : C.amber, sub: verdict ? `${t("verdictField", lang)}: ${verdict}` : t("areaFundamentals", lang) },
//       yld && { lbl: t("grossYield", lang), val: `${yld}%`, valColor: parseFloat(yld) > 6.1 ? C.green : C.amber, sub: t("forInvestorPitch", lang) },
//       avgPsm && { lbl: t("avgPricePerSqm", lang), val: `AED ${parseInt(avgPsm).toLocaleString()}`, valColor: C.textPrimary, sub: t("dldTruvaluBenchmark", lang) },
//       tx && { lbl: t("weeklyDldVolume", lang), val: String(tx), valColor: C.textPrimary, sub: txDelta != null ? `${parseFloat(txDelta) > 0 ? "+" : ""}${txDelta}% ${t("wow", lang)}` : t("liveData", lang) },
//       distress && { lbl: t("distressPctLbl", lang), val: `${distress}%`, valColor: parseFloat(distress) > 10 ? C.red : C.green, sub: t("shareWithInvestors", lang) },
//       trend != null && { lbl: t("priceDirection", lang), val: `${parseFloat(trend) > 0 ? "+" : ""}${trend}% YoY`, valColor: parseFloat(trend) > 0 ? C.green : C.red, sub: parseFloat(trend) > 0 ? t("tellBuyersEntry", lang) : t("tellBuyersNegotiate", lang) },
//     ];
//   } else {
//     items = [
//       verdict && { lbl: t("verdictLbl", lang), val: moodLabel, valColor: moodColor, sub: score ? `${t("scoreLbl", lang)} ${score}/100` : t("marketSignal", lang) },
//       yld && { lbl: t("grossYield", lang), val: `${yld}%`, valColor: parseFloat(yld) > 6.1 ? C.green : C.amber, sub: t("vsDubaiAvg", lang) },
//       avgPsm && { lbl: t("avgPricePerSqm", lang), val: `AED ${parseInt(avgPsm).toLocaleString()}`, valColor: C.textPrimary, sub: t("truvaluBenchmark", lang) },
//       trend != null && { lbl: t("priceTrendLbl", lang), val: `${parseFloat(trend) > 0 ? "+" : ""}${trend}% YoY`, valColor: parseFloat(trend) > 0 ? C.green : C.red, sub: t("yearOnYear", lang) },
//     ];
//   }
...
//   const verdictBg  = s >= 75 ? C.greenL : C.amberL;
//   const comps = [
//     { label: t("qBuying", lang),    val: Math.round(s * 0.87), color: s >= 65 ? C.amber : C.red },
//     { label: t("qPriceFair", lang), val: Math.min(99, Math.round(s * 1.10)), color: C.green },
//     { label: t("qComingNearby", lang), val: Math.min(99, Math.round(s * 1.18)), color: C.green },
//     { label: t("qMoodPositive", lang), val: Math.round(s * 0.62), color: s >= 70 ? C.amber : C.red },
//   ];
//   return (
//   <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 10, padding: "20px 24px", display: "flex", alignItems: "center", gap: 32, flexWrap: "wrap" }}>
//     {/* Left: verdict + score */}
//     <div style={{ textAlign: "center", flexShrink: 0 }}>
//       <div style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".1em", padding: "4px 12px", borderRadius: 20, display: "inline-block", marginBottom: 8, background: verdictBg, color: scoreColor }}>{verdict || (s >= 75 ? t("buy", lang) : s >= 65 ? t("hold", lang) : t("watch", lang))}</div>
//       <div style={{ fontSize: 48, fontWeight: 900, color: scoreColor, lineHeight: 1, letterSpacing: "-.02em" }}>{score}</div>
//       <div style={{ fontSize: 14, color: C.textMuted }}>/100</div>
//       <div style={{ fontSize: 11, color: C.textMuted, marginTop: 4 }}>{t("outlook2026", lang)}</div>
//     </div>
//     {/* Right: component bars */}
//     <div style={{ flex: 1, minWidth: 260, display: "flex", flexDirection: "column", gap: 10 }}>
//       {comps.map((comp, i) => (
...
//    {
//       num: 5,
//       title: t("step5Title", lang),
//     },
//   ];

//   return (
//     <CardSection title={`${t("buyingGuideTitle", lang)} ${area.toUpperCase()}`} badge={t("firstTimeBuyer", lang)}>
//       {steps.map((step, i) => (
//         <div key={step.num} style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "12px 0", borderBottom: i < steps.length - 1 ? `1px solid ${C.border}` : "none" }}>
//           <div style={{ width: 26, height: 26, borderRadius: "50%", background: C.copper, color: "#fff", fontSize: 12, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{step.num}</div>
//           <div>
...
//   const configs = {
//     buyer: {
//       title: t("ptBuyerTitle", lang, { area: area.toUpperCase() }),
//       headers: t("ptBuyerHeaders", lang),
//       row: (br) => {
//         const med = parseFloat(bmed[br] || 0);
//         return [br, fmtAED(Math.round(med * 0.75)), fmtAED(med), fmtAED(Math.round(med * 1.40))];
//       },
//       note: t("ptBuyerNote", lang)
//     },
//     seller: {
//       title: t("ptSellerTitle", lang),
//       headers: t("ptSellerHeaders", lang),
//       row: (br) => {
//         const psm = parseInt(bpsm[br] || 0);
//         const med = parseFloat(bmed[br] || 0);
//         return [br, `AED ${psm.toLocaleString()}`, fmtAED(med), med ? fmtAED(Math.round(med * 1.06)) : "—"];
//       },
//       note: t("ptSellerNote", lang)
//     },
//     investor: {
//       title: t("ptInvestorTitle", lang),
//       headers: t("ptInvestorHeaders", lang),
//       row: (br) => {
//         const psm = parseInt(bpsm[br] || 0);
//         const med = parseFloat(bmed[br] || 0);
//         const rent = med && yld ? fmtAED(Math.round(med * yld / 100)) : "—";
//         return [br, `AED ${psm.toLocaleString()}`, fmtAED(med), rent];
//       },
//       note: t("ptInvestorNote", lang, { yield: yld })
//     },
//     broker: {
//       title: t("ptBrokerTitle", lang),
//       headers: t("ptBrokerHeaders", lang),
//       row: (br) => {
//         const psm = parseInt(bpsm[br] || 0);
//         const med = parseFloat(bmed[br] || 0);
//         return [br, `AED ${psm.toLocaleString()}`, fmtAED(med), med ? fmtAED(Math.round(med * 1.10)) : "—"];
//       },
//       note: t("ptBrokerNote", lang)
//     },
//     general: {
//       title: t("ptGeneralTitle", lang),
//       headers: t("ptGeneralHeaders", lang),
//       row: (br) => {
//         const psm = parseInt(bpsm[br] || 0);
//         const med = parseFloat(bmed[br] || 0);
//         return [br, `AED ${psm.toLocaleString()}`, fmtAED(med), ""];
//       },
//       note: t("ptGeneralNote", lang)
//     }
//   };
...
// // OWNERSHIP COSTS — matches right side of Image 3
// // ─────────────────────────────────────────────────────────────────
// function OwnershipCosts({ msg, lang }) {
//   if (msg.user_type !== "buyer") return null;
//   const stats = msg.transaction_stats || {};
//   const bmed  = stats.median_price_by_bedroom || {};
//   const yld   = parseFloat(msg.yield_pct || 7);
...
//   return (
//     <CardSection title={t("ownershipTitle", lang)}>
//       <StRow label={t("dldTransferFee", lang)}           value={t("pctOfPrice", lang)} />
//       <StRow label={t("agentCommission", lang)}            value={t("negotiable", lang)} />
//       <StRow label={t("annualServiceCharges", lang)}      value={serviceCharge} />
//       <StRow label={t("typicalMaintenance", lang)}  value={t("maintenanceRange", lang)} />
//       {annualRent && <StRow label={`${t("annualRentalIncome", lang)} (${firstBr})`} value={`${fmtAED(Math.round(annualRent * 0.93 / 1000) * 1000)}–${fmtAED(annualRent)}`} valueColor={C.green} />}
//       <StRow label={t("netYieldAfterCharges", lang)} value={`${netYield}%`} valueColor={C.green} />
//       <StRow label={t("mortgageAvailability", lang)}        value={t("ltv80", lang)} last />
//     </CardSection>
//   );
// }
...
// // INVESTOR METRICS — matches Image 4 (4 big cards)
// // ─────────────────────────────────────────────────────────────────
// function InvestorMetrics({ msg, lang }) {
//   if (msg.user_type !== "investor") return null;
//   const intel    = msg.area_intelligence || {};
//   const stats    = msg.transaction_stats || {};
//   const yld      = msg.yield_pct;
...
//  const metrics = [
//     yld && { title: t("grossYield", lang), val: `${yld}%`, color: parseFloat(yld) > 6.1 ? C.green : C.amber, sub: `${t("dubaiAvgLabel", lang)} · ${intel.area_name_en || "Area"} ${parseFloat(yld) > 6.1 ? t("aboveAvg4yr", lang) : t("nearAvg4yr", lang)}` },
//     distress && { title: t("distressOpportunity", lang), val: `${distress}%`, color: C.amber, sub: distressUnits ? `${distressUnits.toLocaleString()} ${t("unitsPricedBelowFloorNow", lang)}` : t("unitsBelowMarketFloor", lang) },
//     catScore && { title: t("catalystScoreLbl", lang), val: `${catScore}/100`, color: parseFloat(catScore) >= 70 ? C.green : C.amber, sub: `${confirmedCats} ${t("confirmedCatalysts", lang)}` },
//     activeProjects && { title: t("offPlanPipeline", lang), val: `${activeProjects} ${t("projectsSuffix", lang)}`, color: C.blue, sub: `${t("activeProjectsIn", lang)} ${intel.area_name_en || "this area"}` },
//   ].filter(Boolean);
...
// // MARKET COMPOSITION — matches left side of Image 5
// // ─────────────────────────────────────────────────────────────────
// function MarketCompositionCard({ msg, lang }) {
//   if (!["investor", "broker"].includes(msg.user_type)) return null;
//   return (
//     <CardSection title={t("marketCompositionTitle", lang)}>
//       <RatioBar left={t("offPlanPrimary", lang)} leftPct={58} leftColor={C.blue} right={t("readySecondary", lang)} rightPct={42} rightColor={C.amber} />
//       <RatioBar left={t("investorOwned", lang)} leftPct={62} leftColor={C.copper} right={t("endUser", lang)} rightPct={38} rightColor={C.green} />
//       <RatioBar left={t("apartments", lang)} leftPct={87} leftColor={C.green} right={t("villasTh", lang)} rightPct={13} rightColor="#7C3AED" />
//       <RatioBar left={t("longTermTenants", lang)} leftPct={88} leftColor="#14B8A6" right={t("shortStay", lang)} rightPct={12} rightColor="#E2E8F0" last />
//     </CardSection>
//   );
// }
// // ─────────────────────────────────────────────────────────────────
// // TRUVALU BENCHMARK TABLE — matches right side of Image 5
// // ─────────────────────────────────────────────────────────────────
// function TruvaluBenchmark({ msg, lang }) {
//   const stats = msg.transaction_stats || {};
//   const bpsm  = stats.bedroom_avg_psm || {};
//   const rows  = ["Studio", "1 BR", "2 BR", "3 BR", "4 BR"].filter(br => bpsm[br]);
//   if (!rows.length) return null;
...
//   return (
//  <CardSection title={t("truvaluBenchmarkTitle", lang)} badge={t("ricsAligned", lang)}>
//       <table style={{ width: "100%", borderCollapse: "collapse" }}>
//         <thead>
//           <tr>{[t("colType",lang), t("colTruvalu",lang), t("colAsking",lang), t("colGap",lang), t("colSignal",lang)].map(h => (
//             <th key={h} style={{ padding: "6px 8px 6px 0", textAlign: "left", fontSize: 9, textTransform: "uppercase", color: C.textMuted, borderBottom: `1px solid ${C.border}`, fontWeight: 700 }}>{h}</th>
//           ))}</tr>
//         </thead>
//         <tbody>
...
//             const ask  = Math.round(truv * (1 + (Math.random() * 0.08 - 0.04)));
//             const gap  = ((ask - truv) / truv * 100).toFixed(1);
//             const signal = parseFloat(gap) > 2 ? { label: t("signalPremium", lang), bg: C.redL, color: C.red } : parseFloat(gap) < -2 ? { label: t("signalOpportunity", lang), bg: C.greenL, color: C.green } : { label: t("signalFair", lang), bg: C.amberL, color: C.amber };
//             return (
//               <tr key={br}>
//                 <td style={{ padding: "8px 8px 8px 0", fontSize: 12, borderBottom: i < rows.length - 1 ? `1px solid ${C.border}` : "none", fontWeight: 600 }}>{br}</td>
//                 <td style={{ padding: "8px 8px 8px 0", fontSize: 12, borderBottom: i < rows.length - 1 ? `1px solid ${C.border}` : "none", fontWeight: 700 }}>AED {truv.toLocaleString()}</td>
...
// // YIELD BY UNIT TYPE — matches bottom right of Image 5
// // ─────────────────────────────────────────────────────────────────
// function YieldByTypeCard({ msg, lang }) {
//   if (!["investor", "broker"].includes(msg.user_type)) return null;
//   const yld = parseFloat(msg.yield_pct || 7);
//   const yieldByType = [
//     { type: "Studio", val: +(yld * 1.19).toFixed(1) },
...
//   return (
//    <CardSection title={t("rentalYieldByType", lang)}>
//       {yieldByType.map(y => (
//         <div key={y.type} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
//           <span style={{ fontSize: 11, width: 52, flexShrink: 0, color: C.textSecondary }}>{y.type}</span>
//           <div style={{ flex: 1, height: 6, background: "#F3F4F6", borderRadius: 3 }}>
...
//         </div>
//       ))}
//       <div style={{ fontSize: 10, color: C.textMuted, textAlign: "right", marginBottom: 8 }}>— {t("dubaiAvgSuffix", lang)}</div>
//       <StRow label={t("bestYieldUnitType", lang)} value={`Studio (${yieldByType[0].val}%)`} valueColor={C.green} />
//       <StRow label={t("fiveYearYieldTrend", lang)}   value={`↑ 6.1% → ${yld}%`} valueColor={C.green} />
//       <StRow label={t("avgDaysToRent", lang)} value={`18 ${t("daysSuffix", lang)}`} last />
//     </CardSection>
//   );
// }
...
// // OWNER / SELLER VALUATION — matches Image 6
// // ─────────────────────────────────────────────────────────────────
// function OwnerValuation({ msg, lang }) {
//   const isMobile = useIsMobile();
//   if (msg.user_type !== "seller") return null;
// const intel = msg.area_intelligence || {};
//   if (!intel.area_name_en) return null;
...
//     <>
//       {/* Valuation banner */}
//      <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".1em", color: C.copper }}>{t("yourAssetValuation", lang)}</div>
//         <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
//           <div>
//             <h2 style={{ fontSize: 18, fontWeight: 900, color: C.copper, margin: "0 0 4px" }}>{firstBr} {t("isWorth", lang) === "is worth" ? `in ${area} is worth` : `في ${area} ${t("isWorth", lang)}`} {fmtAED(low)} — {fmtAED(high)}</h2>
//             <p style={{ fontSize: 12, color: C.textMuted, margin: 0 }}>{t("valuationBasis", lang)}</p>
//           </div>
//           <div style={{ textAlign: "right", flexShrink: 0 }}>
//             <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: ".1em", color: C.textMuted }}>{t("truvaluFairValue", lang)}</div>
//             <div style={{ fontSize: 20, fontWeight: 800, color: C.textPrimary }}>{fmtAED(firstMed)}</div>
//             <div style={{ fontSize: 11, color: C.green }}>↑ +{fmtAED(gain6m)} {t("vs6MonthsAgo", lang)}</div>
//           </div>
//         </div>
...
//       {/* 3 panels */}
//      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10, marginBottom: 12 }}>
//         <CardSection title={t("shouldYouSellNow", lang)}>
//           <div style={{ fontSize: 24, fontWeight: 900, color: trend && parseFloat(trend) > 0 ? C.green : C.amber, marginBottom: 8 }}>
//             {trend && parseFloat(trend) > 0 ? t("yesGoodTime", lang) : t("hold6to12", lang)}
//           </div>
//           <p style={{ fontSize: 12, color: C.textSecondary, lineHeight: 1.6, marginBottom: 12 }}>
//             {trend && parseFloat(trend) > 0
//               ? t("marketRising", lang, { trend })
//               : t("catalystsPushPrices", lang)}
//           </p>
//           <StRow label={t("daysToSellCurrent", lang)} value={`${daysToSell} ${t("daysSuffix", lang)}`} valueColor={daysToSell > 40 ? C.red : C.green} />
//           <StRow label={t("expectedPostCatalyst", lang)} value="8–14%" valueColor={C.green} />
//           <StRow label={t("marketSentiment", lang)} value={trend && parseFloat(trend) > 0 ? t("bullish", lang) : t("cautious", lang)} valueColor={trend && parseFloat(trend) > 0 ? C.green : C.amber} last />
//         </CardSection>
//         <CardSection title={t("shouldYouRentOut", lang)}>
//           <div style={{ fontSize: 24, fontWeight: 900, color: C.green, marginBottom: 8 }}>{t("yesGoodYield", lang)}</div>
//           <p style={{ fontSize: 12, color: C.textSecondary, lineHeight: 1.6, marginBottom: 12 }}>
//             {area}{t("rentalMarketActive", lang, { br: firstBr, rent: fmtAED(annualRent), rentShort: fmtAED(annualRentShort) })}
//           </p>
//           <StRow label={`${t("annualLongTermRent", lang)} (${firstBr})`} value={`${fmtAED(Math.round(annualRent * 0.93 / 1000) * 1000)}–${fmtAED(annualRent)}`} valueColor={C.green} />
//           <StRow label={t("shortTermFurnished", lang)} value={`${fmtAED(annualRent)}–${fmtAED(annualRentShort)}`} valueColor={C.green} />
//           <StRow label={t("avgDaysToRent", lang)} value={`18 ${t("daysSuffix", lang)}`} last />
//         </CardSection>
//       </div>
//     </>
...
// // PRICE HISTORY CHART — matches Image 7
// // ─────────────────────────────────────────────────────────────────
// function PriceHistoryCard({ msg, lang }) {
//   const hist  = msg.price_history || {};
//   const years = Object.keys(hist).sort();
//   if (years.length < 2) return null;
...
//   const area     = intel.area_name_en || "Area";
// const tabLabel = userType === "investor"
//     ? `📈 ${t("priceHistoryInvestor", lang)}`
//     : `📜 ${area.toUpperCase()} ${t("priceHistoryGeneral", lang)}`;
...
//   return (
//     <CardSection title={stripEmojis(tabLabel)} badge={t("truvaluVsDld", lang)}>
//       <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8 }}>
//         <span style={{ fontSize: 12, fontWeight: 700, padding: "3px 9px", borderRadius: 5, background: rising ? C.greenL : C.redL, color: rising ? "#065F46" : "#991B1B" }}>
//           {rising ? "+" : ""}{chgPct}% {t("overYears", lang)} {years.length} {t("yr", lang)}{years.length > 1 && lang === "en" ? "s" : ""}
//         </span>
//       </div>
//       <div style={{ background: "#FAF8F5", borderRadius: 6, padding: "12px 8px 8px" }}>
//         <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block" }}>
...
// // NATIONALITY CARD — matches Images 5 & 10
// // ─────────────────────────────────────────────────────────────────
// function NationalityCard({ msg, lang }) {
//   const intel = msg.area_intelligence || {};
//   const nats  = intel.buyer_nationalities;
//   if (!nats || !nats.length) return null;

//   const badge = nats.some(n => n.pct) ? t("dldVerified", lang) : t("marketEstimate", lang);

//   return (
//     <CardSection title={t("buyerNationality", lang)} badge={badge}>
//       {nats.slice(0, 8).map((n, i) => (
//         <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
//           <span style={{ fontSize: 14, width: 20 }}>{n.flag || "🌍"}</span>
//           <span style={{ fontSize: 12, width: 70, flexShrink: 0, color: C.textSecondary }}>{n.name}</span>
...
// // DISTRESS METER — matches top of Image 9
// // ─────────────────────────────────────────────────────────────────
// function DistressMeter({ msg, lang }) {
//   const distress = msg.distress_pct;
//   const intel    = msg.area_intelligence || {};
//   const area     = intel.area_name_en || "this area";
//   if (!distress) return null;
...
//       <div style={{ fontSize: 28, fontWeight: 900, color: C.amber, flexShrink: 0 }}>{distress}%</div>
//       <div style={{ fontSize: 12, color: C.textSecondary, lineHeight: 1.6 }}>
//        <strong style={{ color: C.textPrimary }}>{t("distressMeterLabel", lang)}</strong> {distressUnits.toLocaleString()} {t("distressListingsBelow", lang, { area })}
//         {parseFloat(distress) > 10 ? " " + t("aboveAvgDriven", lang) : " " + t("nearAvgStable", lang)}
//       </div>
//     </div>
//   );
// }
...
// // ANNUAL RENT RANGES — matches left of Image 10
// // ─────────────────────────────────────────────────────────────────
// function RentRangesCard({ msg, lang }) {
//   if (!["investor", "seller", "broker"].includes(msg.user_type)) return null;
//   const stats = msg.transaction_stats || {};
//   const bmed  = stats.median_price_by_bedroom || {};
//   const yld   = parseFloat(msg.yield_pct || 7);
...
//   return (
//     <CardSection title={t("annualRentRanges", lang)}>
//       <table style={{ width: "100%", borderCollapse: "collapse" }}>
//         <thead>
//           <tr>{[t("colTypeShort",lang), t("colMin",lang), t("colAvg",lang), t("colMax",lang)].map(h => (
//             <th key={h} style={{ padding: "6px 6px 6px 0", textAlign: "left", fontSize: 9, textTransform: "uppercase", color: C.textMuted, borderBottom: `1px solid ${C.border}`, fontWeight: 700 }}>{h}</th>
//           ))}</tr>
//         </thead>
//         <tbody>
...
// // CATALYSTS CARD — matches Image 11 (timeline style)
// // ─────────────────────────────────────────────────────────────────
// function CatalystsCard({ msg, lang }) {
//   const cats     = msg.area_catalysts || [];
//   const userType = msg.user_type || "general";
//   const intel    = msg.area_intelligence || {};
//   const catScore = intel.catalyst_score;
...
//   const label = "🔭 " + ({
//     buyer:    t("catalystsBuyer", lang),
//     seller:   t("catalystsSeller", lang),
//     investor: t("catalystsInvestor", lang),
//     broker:   t("catalystsBroker", lang),
//     general:  t("catalystsGeneral", lang),
//   }[userType] || t("catalystsGeneral", lang));

//   const typeColors = {
//     metro:    { bg: C.blueL, border: "#DBEAFE", dot: C.blue, label: t("typeMetro", lang) },
//     school:   { bg: C.greenL, border: "#BBF7D0", dot: C.green, label: t("typeSchool", lang) },
//     mall:     { bg: C.amberL, border: "#FCD34D", dot: C.amber, label: t("typeRetail", lang) },
//     hospital: { bg: "#FDF4FF", border: "#E9D5FF", dot: "#7C3AED", label: t("typeHealth", lang) },
//     road:     { bg: "#F0F9FF", border: "#BAE6FD", dot: "#0284C7", label: t("typeRoad", lang) },
//     park:     { bg: C.greenL, border: "#BBF7D0", dot: C.green, label: t("typePark", lang) },
//     airport:  { bg: C.blueL, border: "#DBEAFE", dot: C.blue, label: t("typeAirport", lang) },
//   };
//   const confColors = { confirmed: C.green, announced: C.blue, likely: C.amber, spec: C.textMuted };
...
//         <div style={{ position: "absolute", left: 4, top: 6, bottom: 6, width: 2, background: C.border, borderRadius: 1 }} />
//         {cats.slice(0, 4).map((c, i) => {
//           const tc = typeColors[c.catalyst_type] || { bg: C.amberL, border: "#FCD34D", dot: C.amber, label: t("typeProject", lang) };
//           const dateLabel = c.expected_date ? new Date(c.expected_date).toLocaleDateString("en-GB", { month: "short", year: "numeric" }) : "TBC";
//           return (
//             <div key={i} style={{ position: "relative", marginBottom: 18 }}>
//               <div style={{ position: "absolute", left: -24, top: 5, width: 12, height: 12, borderRadius: "50%", background: tc.dot, border: `2px solid #fff` }} />
...
//               {c.description && <div style={{ fontSize: 11, color: C.textMuted, lineHeight: 1.55 }}>{c.description}</div>}
//               <div style={{ fontSize: 11, marginTop: 4, color: C.textMuted }}>
//                 {t("expectedImpact", lang)} <strong style={{ color: C.green }}>
//                   {c.catalyst_type === "metro" ? t("metroImpact", lang) : c.catalyst_type === "school" ? t("schoolImpact", lang) : t("positiveImpact", lang)}
//                 </strong>
//               </div>
//             </div>
//           );
//         })}
//        {!cats.length && <div style={{ fontSize: 12, color: C.textMuted, padding: "10px 0" }}>{t("noCatalystsYet", lang)}</div>}
//       </div>
//       {catScore && (
//         <div style={{ marginTop: 10, display: "flex", alignItems: "center", justifyContent: "space-between", background: C.pageBg, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 14px" }}>
//           <div style={{ fontSize: 12, color: C.textSecondary }}>{t("catalystScoreCard", lang)}</div>
//           <div style={{ fontSize: 28, fontWeight: 900, color: parseFloat(catScore) >= 70 ? C.green : C.amber }}>{catScore}/100</div>
//         </div>
//       )}
//     </div>
...
//               <>
//             {/* Score card on top, hero stats below */}
// {msg.score && <div style={{ marginBottom: 12 }}><ScoreCard msg={msg} lang={lang} /></div>}
// <HeroStatsRow msg={msg} lang={lang} />

//                 {/* Buyer: Guide + Price table + Costs */}
//                 <BuyerGuide msg={msg} lang={lang} />
//                 <PriceTable msg={msg} lang={lang} />
//                 <OwnershipCosts msg={msg} lang={lang} />

//                 {/* Seller: Owner valuation */}
//                 <OwnerValuation msg={msg} lang={lang} />
...
//                     <YieldByTypeCard msg={msg} lang={lang} />
//                   </div>
//                 )}
...
//                   tabs={[
//                     {
//                       label: t("tabPast", lang),
//                       icon: "📜",
//                       content: (
//                         <>
//                           <PriceHistoryCard msg={msg} lang={lang} />
//                         <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10 }}>
//                             <AreaMaturityCard msg={msg} lang={lang} />
//                             <DeveloperTrackRecordCard msg={msg} lang={lang} />
//                           </div>
//                         </>
//                       ),
//                     },
//                     {
//                       label: t("tabPresent", lang),
//                       icon: "📡",
//                       content: (
//                         <>
...
//                     },
//                     {
//                       label: t("tabFuture", lang),
//                       icon: "🔭",
//                       content: (
//                         (msg.area_catalysts?.length > 0 || msg.area_intelligence?.catalyst_score) ? (
//                           <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: 10, padding: "16px 18px", marginBottom: 12 }}>
//                             <CatalystsCard msg={msg} lang={lang} />
//                           </div>
//                         ) : (
//                           <p style={{ fontSize: 13, color: C.textMuted, padding: "20px 0", textAlign: "center" }}>{t("noCatalystData", lang)}</p>
//                         )
//                       ),
//                     },
//                   ]}
...
//         {msg.area_links && msg.area_links.length > 0 && (
//           <div style={{ marginTop: 16 }}>
//             <div style={{ fontSize: 11, fontWeight: 600, color: C.textLight, marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.6 }}>{t("exploreAreas", lang)}</div>
//             <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
//               {msg.area_links.map((link, i) => (
//                 <a key={i} href={link.url} target="_blank" rel="noopener noreferrer"
//                   style={{ padding: "5px 11px", background: C.copperTint, border: `1px solid ${C.copperBorder}`, borderRadius: 20, color: C.copper, fontSize: 12, fontWeight: 600, textDecoration: "none" }}
...
//           fd.append("file", blob, `query.${ext}`);
//           const res = await fetch(`${BACKEND}/intelligence/transcribe`, {
//             method: "POST",
//             body: fd,
//           });
//           const json = await res.json();
...
//     // Save every logged-in user's query to Supabase
//     if (isBroker && user) {
//       supabase.from("broker_queries").insert({
//         user_id: user.id,
//         email: user.email || user.user_metadata?.email || null,
...
//         page: "/broker",
//       }).then(({ error }) => {
//         if (error) console.error("broker_queries insert:", error.message);
//       });
//     }
...
//   (msg.area_links && msg.area_links.length > 0)
// );
// const isSpecificAnswer = msg.response_mode === "specific_answer";
...
// // Save every logged-in user's query to Supabase, and remember its id
//     // so we can attach the AI's response once it comes back.
//     let queryRowId = null;
//     if (isBroker && user) {
...
//         .select("id")
//         .single();
//       if (qErr) console.error("broker_queries insert:", qErr.message);
//       else queryRowId = qRow?.id ?? null;
//     }
...
function LoginModal({ open, onClose, navigate, returnPath }) {
  if (!open) return null;
  return (
    <div onClick={onClose}
      style={{ position: "fixed", inset: 0, background: "rgba(17,24,39,0.55)",
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <button onClick={() => navigate("/complete-profile", { state: { from: returnPath } })}
          style={{ width: "100%", padding: "13px 16px", borderRadius: 12, border: "none", cursor: "pointer",
            background: "linear-gradient(180deg, #c97d24 0%, #a5620f 100%)", color: "#fff",
            fontWeight: 800, fontSize: 15, fontFamily: "inherit" }}>
          Sign In →
...
`}</style>
  ...
  <>
    ...
    >
      ...
      <LoginModal open={showLoginModal} onClose={() => setShowLoginModal(false)} navigate={navigate} returnPath={location.pathname} />
    </div>
  );
}
