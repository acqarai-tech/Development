    ...
    <div style={{ display: "flex", gap: 12, marginBottom: 28, alignItems: "flex-start" }}>
      ...
      <div style={{ flex: 1, minWidth: 0, paddingTop: 2 }}>
       ...
       {msg.insight && (
        ...
        {msg.lifestyle_areas && msg.lifestyle_areas.length > 0 ? (
          <div style={{ marginTop: 20 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: C.textLight, marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.6 }}>
              Area Breakdown
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {msg.lifestyle_areas.map((area, i) => (
                <div key={i} style={{ border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden" }}>

                  {/* Header */}
                  <div style={{ padding: "14px 16px", background: C.copperTint, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 700, color: C.textPrimary }}>{area.name}</div>
                      <div style={{ fontSize: 11, color: C.textLight, marginTop: 2 }}>
                        {area.master_dev && `Master dev: ${area.master_dev}`}
                        {area.year_est && ` · Est. ${area.year_est}`}
                        {area.completion && ` · ${area.completion}`}
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {area.score && <span style={{ padding: "4px 10px", background: "#F3F4F6", borderRadius: 6, fontSize: 12, fontWeight: 700, color: C.textPrimary }}>Score {area.score}/100</span>}
                      {area.verdict && <span style={{ padding: "4px 10px", borderRadius: 6, fontSize: 12, fontWeight: 700, background: area.verdict === "BUY" ? "#D1FAE5" : "#FEF3C7", color: area.verdict === "BUY" ? "#065F46" : "#92400E" }}>{area.verdict}</span>}
                      {area.ranking && <span style={{ padding: "4px 10px", background: C.copperTint, border: `1px solid ${C.copperBorder}`, borderRadius: 6, fontSize: 12, fontWeight: 700, color: C.copper }}>#{area.ranking} Dubai</span>}
                    </div>
                  </div>
                  ...
                  <div style={{ padding: 16 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 14 }}>
                      {[
                        { label: "Gross Yield", value: area.yield_pct ? `${area.yield_pct}%` : "—", color: "#065F46" },
                        { label: "Price Trend", value: area.price_trend != null ? `${area.price_trend > 0 ? "+" : ""}${area.price_trend}%` : "—", color: area.price_trend > 0 ? "#065F46" : "#991B1B" },
                        { label: "Avg PSM", value: area.truvalu_psm ? `AED ${Math.round(area.truvalu_psm).toLocaleString()}` : "—", color: C.textPrimary },
                      ].map((m, mi) => (
                        <div key={mi} style={{ padding: "10px 8px", background: "#F9FAFB", borderRadius: 8, textAlign: "center" }}>
                          <div style={{ fontSize: 10, color: C.textMuted, marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }}>{m.label}</div>
                    ...
                    {/* Median prices */}
                    {area.median_prices && Object.keys(area.median_prices).length > 0 && (
                      <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: C.textLight, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>💰 Median Prices (DLD)</div>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          {Object.entries(area.median_prices).map(([br, price]) => (
                            <div key={br} style={{ padding: "5px 10px", background: "#F3F4F6", borderRadius: 6, fontSize: 12 }}>
                              <span style={{ color: C.textLight }}>{br}: </span>
                              <span style={{ fontWeight: 700, color: C.textPrimary }}>
                                {price >= 1000000 ? `AED ${(price/1000000).toFixed(2)}M` : `AED ${Math.round(price/1000)}K`}
                              </span>
                            </div>
                          ))}
                        </div>
                    ...
                    {/* Nationalities */}
                    {area.nationalities && area.nationalities.length > 0 && (
                      <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: C.textLight, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>🌍 Who Lives Here</div>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          {area.nationalities.slice(0, 6).map((nat, ni) => (
                            <span key={ni} style={{ padding: "4px 10px", background: C.copperTint, border: `1px solid ${C.copperBorder}`, borderRadius: 20, fontSize: 11, color: C.copper, fontWeight: 600 }}>{nat}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    ...
                    {/* Catalysts */}
                    {area.catalysts && area.catalysts.length > 0 && (
                      <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: C.textLight, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>⚡ Upcoming Catalysts</div>
                        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                          {area.catalysts.map((cat, ci) => (
                            <div key={ci} style={{ fontSize: 12, color: C.textSecondary, display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                              <span style={{ width: 6, height: 6, borderRadius: "50%", background: cat.confidence === "confirmed" ? "#065F46" : C.copper, flexShrink: 0, display: "inline-block" }} />
                              <span style={{ fontWeight: 600, color: C.textPrimary }}>{cat.name}</span>
                              {cat.date && <span style={{ color: C.textMuted }}>· {new Date(cat.date).toLocaleDateString("en-GB", { month: "short", year: "numeric" })}</span>}
                              <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 4, background: cat.confidence === "confirmed" ? "#D1FAE5" : "#FEF3C7", color: cat.confidence === "confirmed" ? "#065F46" : "#92400E" }}>{cat.confidence}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {/* Off-plan */}
                    {(area.active_projects > 0 || area.project_names?.length > 0) && (
                      <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: C.textLight, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>🏗️ Off-Plan Projects</div>
                        <div style={{ fontSize: 12, color: C.textSecondary }}>
                          <span style={{ fontWeight: 700, color: C.textPrimary }}>{area.active_projects || area.project_names?.length} active projects</span>
                          {area.project_names?.length > 0 && <span> · {area.project_names.slice(0, 3).join(", ")}{area.project_names.length > 3 ? "…" : ""}</span>}
                        </div>
                      </div>
                    )}

                    {/* Price history bars */}
                    {area.price_history && Object.keys(area.price_history).length > 0 && (
                      <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: C.textLight, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>📈 Price History (AED/sqm)</div>
                        <div style={{ display: "flex", gap: 4, alignItems: "flex-end", height: 50 }}>
                          {Object.entries(area.price_history).map(([year, psm], pi, arr) => {
                            const max = Math.max(...arr.map(([, v]) => v));
                            const pct = Math.round((psm / max) * 100);
                            return (
                              <div key={year} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                                <div style={{ width: "100%", background: C.copper, borderRadius: "3px 3px 0 0", height: `${pct}%`, minHeight: 4, opacity: 0.7 + (pi / arr.length) * 0.3 }} />
                                <div style={{ fontSize: 9, color: C.textMuted }}>{year}</div>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    {/* Community stats */}
                    <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 14, fontSize: 11, color: C.textMuted }}>
                      {area.res_units && <span>🏠 {Number(area.res_units).toLocaleString()} units</span>}
                      {area.active_projects > 0 && <span>🏗️ {area.active_projects} off-plan</span>}
                      {area.distress_pct && <span>📉 {area.distress_pct}% distress</span>}
                      {area.tx_7d && <span>📊 {area.tx_7d} deals/week</span>}
                      {area.parks && <span>🌳 {area.parks}</span>}
                    </div>

                    {/* CTA */}
                    <a href={area.area_url} target="_blank" rel="noopener noreferrer"
                      style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px", background: C.textPrimary, color: "#fff", borderRadius: 7, fontSize: 12, fontWeight: 700, textDecoration: "none" }}
